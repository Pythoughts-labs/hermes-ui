...
|---|---|---|
| `PORT` | `6060` | UI listen port |
| `BIND_HOST` | `0.0.0.0` | Optional UI bind host. Defaults to IPv4 for stable WSL/Windows access. Set `::` explicitly if you want IPv6 listening. |
...
| `UI_IMAGE` | `hermes-ui-local:latest` | UI image (set to `pythoughtslabs/hermes-ui` to use pre-built) |
...
| `${HERMES_DATA_DIR}/hermes-ui` | UI data (auth token, etc.) |

- Hermes data persists in `./hermes_data`, mapped to `/home/agent/.hermes` in the container.
- UI data persists in `./hermes_data/hermes-ui/`, mapped to `/home/agent/.hermes-ui` in the container.
...
# or
cat ./hermes_data/hermes-ui/.token
```
