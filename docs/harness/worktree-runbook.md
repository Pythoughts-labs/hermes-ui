...
git worktree add -b codex/<short-topic> ../worktrees/hermes-ui-<short-topic> origin/main
cd ../worktrees/hermes-ui-<short-topic>
```
...
```bash
export PORT=18648
export HERMES_UI_HOME="$PWD/.tmp/hermes-ui"
export HERMES_UI_STATE_DIR="$HERMES_UI_HOME"
export UPLOAD_DIR="$PWD/.tmp/uploads"
npm run dev
```

Do not point `HERMES_UI_HOME` at a user's real `~/.hermes-ui` when a task
...
```bash
git worktree remove ../worktrees/hermes-ui-<short-topic>
```
