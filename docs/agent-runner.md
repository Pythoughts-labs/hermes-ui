...
gateway. The goal is to make Claude Code, Codex, and internal UI runs share
...
- Provide shared protocol plumbing that can be used by:
  ...
  - Hidden Coding Agent sessions launched from the UI Chat surface.
...
- Keep existing session storage under the current UI SQLite session tables.
...
- `run-chat/handle-bridge-run.ts` is the active UI Hermes bridge path.
...
- Socket.IO event payloads for UI chat.
...
External proxy requests do not naturally know the UI session. The first
...
- Internal UI API runs reuse the existing `run-chat/response-stream.ts`
...
session binding. Internal UI runs already persist through
`run-chat/response-stream.ts`.
