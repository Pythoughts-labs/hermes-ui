...
Hermes UI now supports turn-based voice input inside the existing chat UI. The current design is intentionally simple: record one voice turn, transcribe it, place the transcript into the chat input for review/editing, and let the user send it with the normal Send button.

## What works today

- Voice output / TTS: assistant replies can be spoken with the existing message playback stack. Server-backed providers are supported through the UI backend, and browser Web Speech remains available as a local playback option.
...
The current UI voice dialogue does not yet include:
...
Those are future areas, but they are outside the current UI voice dialogue scope.