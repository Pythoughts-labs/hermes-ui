...
- Gateway, UI, script, and background-review memory writes are staged for
  later review.
...
Hermes UI already handles dangerous command approvals through
`approval.requested` / `approval.resolved` events and renders an approval bar in
chat and group chat.

Write Gate is different for UI sessions. Hermes Agent does not always emit a
...
- Reviewing those pending writes currently depends on Hermes Agent's
  ...
  inspection.
- A first-class UI popup or review panel would require UI API/UI work
