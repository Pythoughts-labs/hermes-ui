...
This document records the current Workflow page implementation in Hermes UI.
...
- If no workspace is selected, the server creates a default workspace:
  `~/.hermes-ui/workflow/<profile>/<workflowId>`.
...
`workflow` is a UI session source and scheduler marker. It is not a new
