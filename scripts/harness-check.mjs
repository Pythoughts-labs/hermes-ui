...
const uiReleaseWorkflow = await readText('.github/workflows/ui-release.yml')
...
const desktopUiServer = await readText('packages/desktop/src/main/ui-server.ts')
...
for (const [file, text] of [
  ['ui-release.yml', uiReleaseWorkflow],
...
if (!uiReleaseWorkflow.includes('make_latest: false')) {
  fail('ui-release.yml must not mark release uploads as GitHub latest')
}
...
for (const phrase of [
  ...
  if (!desktopUiServer.includes(phrase)) {
    fail(`desktop ui server must expose bundled browser runtime: ${phrase}`)
  }
}

if (desktopUiServer.includes('bundledBrowserExecutable()')) {
  fail('desktop ui server must let agent-browser resolve the bundled browser from AGENT_BROWSER_HOME')
}
...
for (const phrase of [
  ...
  if (!desktopInstallerScript.includes(phrase)) {
    fail(`desktop installer must close stale Hermes UI processes by installed executable path: ${phrase}`)
  }
}

for (const phrase of [
  'https://github.com/Pythoughts-labs/hermes-ui/releases/latest/download',
]) {
  if (!desktopUpdater.includes(phrase)) {
    fail(`desktop updater must point at GitHub releases feed: ${phrase}`)
  }
}
