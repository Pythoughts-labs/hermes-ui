...
const UI_HOME = process.env.HERMES_UI_HOME?.trim()
  ? resolve(process.env.HERMES_UI_HOME.trim())
  : resolve(homedir(), '.hermes-ui')
const PID_DIR = UI_HOME
const PID_FILE = join(PID_DIR, 'server.pid')
const LOG_FILE = join(PID_DIR, 'server.log')
const TOKEN_FILE = join(PID_DIR, '.token')
const LOGIN_LOCK_FILE = join(UI_HOME, '.login-lock.json')
const UI_DB_FILE = join(UI_HOME, 'hermes-ui.db')
...
function getDaemonStopGraceMs(options = {}) {
  ...
  if (restart && shouldPreserveBridgeOnShutdown()) {
    return envPositiveInt('HERMES_UI_RESTART_GRACE_MS') ?? DEFAULT_RESTART_GRACE_MS
  }
  if (restart) {
    return envPositiveInt('HERMES_UI_RESTART_GRACE_MS')
      ?? envPositiveInt('HERMES_UI_STOP_GRACE_MS')
      ?? DEFAULT_STOP_GRACE_MS
  }
  return envPositiveInt('HERMES_UI_STOP_GRACE_MS') ?? DEFAULT_STOP_GRACE_MS
}
...
function getGlobalCliBin() {
  ...
  return process.platform === 'win32'
    ? join(prefix, 'hermes-ui.cmd')
    : join(prefix, 'bin', 'hermes-ui')
}
...
function enableClientMode() {
  process.env.HERMES_UI_DISABLE_GATEWAY_AUTOSTART = '1'
...
function startDaemon(port) {
  ...
  if (existing && isRunning(existing)) {
    console.log(`  ✗ hermes-ui is already running (PID: ${existing})`)
    console.log(`    Use "hermes-ui stop" to stop it first`)
    process.exit(1)
  }
  removePid()
  ...
  console.log(`  ⏳ Starting hermes-ui (PID: ${child.pid}, port: ${port})...`)

  function poll() {
    waited += interval
    if (!isRunning(child.pid)) {
      console.log('  ✗ Failed to start hermes-ui')
...
function stopDaemon(options = {}) {
  ...
  if (pidFromFile && !isRunning(pidFromFile)) {
    removePid()
    console.log(`  ✓ hermes-ui was not running (cleaned stale PID: ${pidFromFile})`)
    pidFromFile = null
  ...
  if (!pid) {
    ...
    if (stoppedPreviewPids) {
      console.log(`  ✓ hermes-ui preview stopped`)
      return
    }
    console.log('  ✗ hermes-ui is not running')
    process.exit(1)
  }

  if (!isRunning(pid)) {
    removePid()
    console.log(`  ✓ hermes-ui was not running (cleaned stale PID)`)
    return
  }
  ...
  try {
    ...
    removePid()
    console.log(`  ✓ hermes-ui stopped (PID: ${pid})`)
  } catch (err) {
...
function showStatus() {
  ...
  if (pid && isRunning(pid)) {
    console.log(`  ✓ hermes-ui is running (PID: ${pid})`)
...
function clearLoginLocks(options = {}) {
  ...
  if (!silent && serverRunning) {
    console.log('  ⚠ hermes-ui is running; restart it to clear in-memory login locks.')
    console.log('    Run: hermes-ui restart')
  }
...
async function resetDefaultLogin(options = {}) {
  ...
  mkdirSync(UI_HOME, { recursive: true })
  const { DatabaseSync } = await import('node:sqlite')
  const db = new DatabaseSync(UI_DB_FILE)
  try {
    db.exec(`
    ...
    if (existing?.id) {
      ...
      if (!silent) {
        ...
        console.log(`    Database: ${UI_DB_FILE}`)
      }
      return { path: UI_DB_FILE, username: DEFAULT_USERNAME, password: DEFAULT_PASSWORD, action: 'updated' }
    }
    ...
    return { path: UI_DB_FILE, username: DEFAULT_USERNAME, password: DEFAULT_PASSWORD, action: 'created' }
  } finally {
    db.close()
  }
}
...
async function main() {
  ...
  if (['-v', '--version', 'version'].includes(command)) {
    console.log(`hermes-ui v${VERSION}`)
    process.exit(0)
  }
...
function doUpdate() {
  console.log('  ⬆ Updating hermes-ui...')
...
function runUpdateInstall(npm) {
  const child = spawnCli(npm, ['install', '-g', '@pythoughts/hermes-ui@latest'], {
    stdio: 'inherit',
    windowsHide: true,
    env: getCurrentNodeEnv(),
  })
