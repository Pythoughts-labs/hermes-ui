...
const DEFAULT_PORT = process.env.HERMES_UI_PORT || process.env.PORT || '8648'
const DEFAULT_BASE_URL = `http://127.0.0.1:${DEFAULT_PORT}`
const DISPLAY_COMMAND = 'hermes-ui-mcp'
...
Environment:
  HERMES_UI_URL       UI base URL. Default: ${DEFAULT_BASE_URL}
  HERMES_UI_HOME      UI state directory. Default: ~/.hermes-ui
  HERMES_UI_STATE_DIR  Fallback UI state directory.
  HERMES_UI_PROFILE   Default Hermes profile when a tool call omits profile.
  HERMES_UI_TOKEN     Optional explicit API token.
...
function appHome() {
  return process.env.HERMES_UI_HOME ||
    process.env.HERMES_UI_STATE_DIR ||
    join(homedir(), '.hermes-ui')
}
...
function readToken(tokenOverride, allowTokenFile = true, profile = '') {
  const explicit = tokenOverride || process.env.HERMES_UI_TOKEN || process.env.AUTH_TOKEN
...
function authHint() {
  return `UI token was not accepted. Pass the current Hermes profile argument so this MCP server can read its temporary token, pass an explicit token argument, or set HERMES_UI_TOKEN.`
}

function baseUrl() {
  return (process.env.HERMES_UI_URL || DEFAULT_BASE_URL).replace(/\/$/, '')
}
...
function appendQuery(path, query) {
  ...
  const parsed = new URL(path, 'http://hermes-ui.local')
...
function normalizeApiPath(path) {
  ...
  const parsed = new URL(raw, 'http://hermes-ui.local')
...
function pathWithoutQuery(path) {
  return new URL(path, 'http://hermes-ui.local').pathname
}
...
const moduleHints = {
  'API Docs': {
    purpose: 'Discover the UI API catalog and generated OpenAPI metadata.',
    keywords: ['operation manual', 'api docs', 'endpoint docs', 'openapi', 'route catalog'],
  },
  Auth: {
    purpose: 'Manage UI authentication state and tokens.',
  ...
  Config: {
    purpose: 'Read and update Hermes UI configuration.',
  ...
  MCP: {
    purpose: 'Manage MCP servers, tools, and UI MCP integration.',
...
function compactOpenApiDocument(openapi, args = {}) {
  ...
  return {
    title: openapi?.info?.title || 'Hermes UI API',
    version: openapi?.info?.version || '',
    usage: hasFilters
      ? 'Use the selected operation details to call hermes_ui_api_request with method, path, query, and body. Auth and profile are handled by the MCP server.'
      : 'This catalog is large. First read module purpose and keywords to choose the right module, then call hermes_ui_api_openapi_get again with tag, path, or method filters to fetch endpoint details on demand.',
...
const authArgumentProperties = {
  token: {
    type: 'string',
    description: 'Optional Hermes UI bearer token. Usually omit this and pass profile so the MCP server can read the temporary profile token.',
  },
  profile: {
    type: 'string',
    description: 'Hermes profile name for profile-scoped UI requests and temporary profile token lookup.',
  },
}
...
const tools = [
  {
    name: 'hermes_ui_api_openapi_get',
    toolset: 'api',
    description: 'Return Hermes UI API documentation as compact JSON. When the user asks to read/check the operation manual, API docs, or endpoint docs, call this tool without filters first to get the outline/module index. Without filters, returns only module purpose, keywords, and operation counts because the full API catalog is large. For endpoint details, call again with tag, path, or method filters, then use hermes_ui_api_request.',
  ...
  {
    name: 'hermes_ui_api_request',
    toolset: 'api',
    description: 'Execute a Hermes UI operation by calling an endpoint path. Use hermes_ui_api_openapi_get first as the operation manual to inspect method, parameters, requestBody, and responses.',
    inputSchema: inputSchema({
        method: {
          type: 'string',
          enum: ['GET', 'POST', 'PUT', 'PATCH', 'DELETE', 'HEAD'],
        ...
        path: {
          type: 'string',
          description: 'Relative Hermes UI endpoint path from the operation manual, for example /api/hermes/sessions?limit=20. Full URLs and // paths are rejected.',
        },
        body: {
  ...
  {
    name: 'hermes_ui_use_chat_run',
    toolset: 'use',
    description: 'Start one Hermes UI chat or coding-agent run through the HTTP bridge and wait for completion.',
    inputSchema: inputSchema({
        input: {
          oneOf: [
            { type: 'string' },
  ...
  {
    name: 'hermes_ui_use_sessions_list',
    toolset: 'use',
    description: 'List Hermes UI chat sessions.',
    inputSchema: inputSchema({
        limit: {
          type: 'number',
  ...
  {
    name: 'hermes_ui_use_sessions_count',
    toolset: 'use',
    description: 'Count Hermes UI chat sessions without returning the session list.',
    inputSchema: inputSchema({
        source: {
          type: 'string',
          description: 'Optional session source filter.',
  ...
  {
    name: 'hermes_ui_use_usage_stats',
    toolset: 'use',
    description: 'Query Hermes UI usage totals, cost estimate, model breakdown, and daily trend for the selected profile.',
    inputSchema: inputSchema({
        days: {
          type: 'number',
  ...
  {
    name: 'hermes_ui_use_session_get',
    toolset: 'use',
    description: 'Get one Hermes UI session by id.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_session_messages',
    toolset: 'use',
    description: 'Get messages for one Hermes UI conversation. By default returns user and assistant messages only.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_session_context',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_session_delete',
    toolset: 'use',
    description: 'Delete one Hermes UI session by id.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_session_rename',
    toolset: 'use',
    description: 'Rename one Hermes UI session title.',
    inputSchema: inputSchema({
        session_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_profiles_list',
    toolset: 'use',
    description: 'List Hermes UI profiles.',
    inputSchema: inputSchema(),
  },
  {
    name: 'hermes_ui_use_available_models',
    toolset: 'use',
    description: 'List available Hermes UI models for the selected profile.',
    inputSchema: inputSchema(),
  },
  {
    name: 'hermes_ui_use_model_provider_get',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_provider_add',
    toolset: 'use',
    description: 'Add or update a Hermes UI model provider for the selected profile, then make it the active default provider/model.',
    inputSchema: inputSchema({
        name: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_provider_delete',
    toolset: 'use',
    description: 'Delete a Hermes UI model provider or clear a built-in provider credential for the selected profile.',
    inputSchema: inputSchema({
        pool_key: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_worker_status',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_workflows_list',
    toolset: 'use',
    description: 'List Hermes UI workflows for the selected or requested profile.',
    inputSchema: inputSchema({
        profile: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_workflow_get',
    toolset: 'use',
    description: 'Get one Hermes UI workflow by id.',
    inputSchema: inputSchema({
        workflow_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_workflow_create',
    toolset: 'use',
    description: 'Create a Hermes UI workflow with optional nodes, edges, viewport, workspace, and profile.',
    inputSchema: inputSchema({
        name: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_workflow_update',
    toolset: 'use',
    description: 'Update a Hermes UI workflow name, workspace, nodes, edges, or viewport.',
    inputSchema: inputSchema({
        workflow_id: {
          type: 'string',
          description: 'Workflow id.',
  ...
  {
    name: 'hermes_ui_use_workflow_delete',
    toolset: 'use',
    description: 'Delete one Hermes UI workflow by id, including its workflow run records.',
    inputSchema: inputSchema({
        workflow_id: {
          type: 'string',
  ...
  {
    name: 'hermes_ui_use_workflow_runs_list',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_workflow_run_start',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_workflow_run_stop',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_workflow_run_rerun_from_node',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_use_workflow_run_delete',
    toolset: 'use',
  ...
  {
    name: 'hermes_ui_lan_terminal_create',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_terminal_list',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_terminal_input',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_terminal_read',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_terminal_resize',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_terminal_close',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_command_exec',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_file_download',
    toolset: 'devices',
  ...
  {
    name: 'hermes_ui_lan_file_upload',
    toolset: 'devices',
...
const TOOL_ALIASES = new Map([
  ['hermes_api_openapi_get', 'hermes_ui_api_openapi_get'],
  ['hermes_api_request', 'hermes_ui_api_request'],
  ['hermes_lan_devices_list', 'hermes_ui_lan_devices_list'],
  ['hermes_lan_devices_scan', 'hermes_ui_lan_devices_scan'],
  ['hermes_lan_peer_connect', 'hermes_ui_lan_peer_connect'],
  ['hermes_lan_peer_connections', 'hermes_ui_lan_peer_connections'],
  ['hermes_lan_peer_disconnect', 'hermes_ui_lan_peer_disconnect'],
  ['hermes_lan_terminal_create', 'hermes_ui_lan_terminal_create'],
  ['hermes_lan_terminal_list', 'hermes_ui_lan_terminal_list'],
  ['hermes_lan_terminal_input', 'hermes_ui_lan_terminal_input'],
  ['hermes_lan_terminal_read', 'hermes_ui_lan_terminal_read'],
  ['hermes_lan_terminal_resize', 'hermes_ui_lan_terminal_resize'],
  ['hermes_lan_terminal_close', 'hermes_ui_lan_terminal_close'],
  ['hermes_lan_command_exec', 'hermes_ui_lan_command_exec'],
  ['hermes_lan_file_download', 'hermes_ui_lan_file_download'],
  ['hermes_lan_file_upload', 'hermes_ui_lan_file_upload'],
])
...
async function callTool(name, args = {}) {
  ...
  switch (resolveToolName(name)) {
    ...
    case 'hermes_ui_api_request': {
      ...
      if (!path) return errorText('Invalid path. Use a relative /api/... or /health path from hermes_ui_api_openapi_get; full URLs are not allowed.')
      const validationError = await validateApiRequest(method, path, args)
      if (validationError) return errorText(`Invalid API request for ${method} ${pathWithoutQuery(path)}: ${validationError}. Use hermes_ui_api_openapi_get to inspect required parameters and requestBody.`)
    ...
    case 'hermes_ui_use_session_context':
    ...
    case 'hermes_ui_use_provider_delete':
    ...
    case 'hermes_ui_use_workflow_update':
      return jsonText(await request(`/api/hermes/workflows/${encodeURIComponent(args.workflow_id)}`, withAuthArgs(args, {
        method: 'PATCH',
        body: pickDefined(args, [
          'name',
    ...
    case 'hermes_ui_use_workflow_delete':
      return jsonText(await request(`/api/hermes/workflows/${encodeURIComponent(args.workflow_id)}`, withAuthArgs(args, {
        method: 'DELETE',
      })))
    case 'hermes_ui_use_workflow_runs_list':
    ...
    case 'hermes_ui_use_workflow_run_start':
    ...
    case 'hermes_ui_use_workflow_run_stop':
    ...
    case 'hermes_ui_use_workflow_run_rerun_from_node':
    ...
    case 'hermes_ui_use_workflow_run_delete':
    ...
    case 'hermes_ui_lan_terminal_create':
    ...
    case 'hermes_ui_lan_terminal_list':
    ...
    case 'hermes_ui_lan_terminal_input':
    ...
    case 'hermes_ui_lan_terminal_read':
    ...
    case 'hermes_ui_lan_terminal_resize':
    ...
    case 'hermes_ui_lan_terminal_close':
    ...
    case 'hermes_ui_lan_command_exec':
    ...
    case 'hermes_ui_lan_file_download':
    ...
    case 'hermes_ui_lan_file_upload':
