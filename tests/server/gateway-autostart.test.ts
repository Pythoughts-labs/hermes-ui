...
describe('gateway autostart status parsing', () => {
  ...
  it('honors UI gateway management overrides around Hermes Agent multiplex config', () => {
  ...
  it('detects the environment-level gateway autostart disable flag', () => {
    expect(gatewayAutostartDisabledByEnv({ HERMES_UI_DISABLE_GATEWAY_AUTOSTART: '1' })).toBe(true)
    expect(gatewayAutostartDisabledByEnv({ HERMES_UI_DISABLE_GATEWAY_AUTOSTART: 'true' })).toBe(true)
    expect(gatewayAutostartDisabledByEnv({ HERMES_UI_DISABLE_GATEWAY_AUTOSTART: 'off' })).toBe(false)
...
`
  ...
  it('uses managed gateway mode by default', () => {
    const previous = process.env.HERMES_UI_MANAGED_GATEWAY
    try {
      delete process.env.HERMES_UI_MANAGED_GATEWAY
      ...
      if (previous === undefined) delete process.env.HERMES_UI_MANAGED_GATEWAY
      else process.env.HERMES_UI_MANAGED_GATEWAY = previous
    }
  })

  it('keeps managed gateway mode enabled when explicitly set', () => {
    const previous = process.env.HERMES_UI_MANAGED_GATEWAY
    process.env.HERMES_UI_MANAGED_GATEWAY = '1'
    try {
      expect(shouldUseManagedGatewayRun()).toBe(true)
  ...
  it('allows managed gateway mode to be disabled by environment', () => {
    ...
    try {
      for (const value of ['0', 'false', 'no', 'off']) {
        process.env.HERMES_UI_MANAGED_GATEWAY = value
  ...
  it('only recovers Windows desktop gateway orphans when enabled', () => {
    ...
    expect(shouldRecoverWindowsDesktopGatewayOrphans('win32', {
      HERMES_DESKTOP: 'true',
      HERMES_UI_DISABLE_GATEWAY_STARTUP_RECOVERY: '1',
    })).toBe(false)
  })
