...
describe('session conversations controller', () => {
  ...
  it('marks Hermes history sessions that already exist in the UI store', async () => {
    ...
    expect(ctx.body.sessions).toEqual([
      expect.objectContaining({ id: 'cli-1', profile: 'travel', ui_imported: true }),
      expect.objectContaining({ id: 'cli-2', profile: 'travel', ui_imported: false }),
    ])
  })
  ...
  it('keeps archived sessions visible in Hermes history', async () => {
    ...
    expect(ctx.body.sessions).toEqual([
      expect.objectContaining({ id: 'cli-archived', profile: 'travel', ui_imported: true }),
    ])
  })
  ...
  it('keeps archived coding-agent sessions visible in Hermes history', async () => {
    ...
    expect(ctx.body.sessions).toEqual([
      expect.objectContaining({ id: 'codex-archived', source: 'coding_agent', agent: 'codex', ui_imported: true }),
    ])
  })
  ...
  it('deletes a current-profile Hermes history session even when no local UI session exists', async () => {
  ...
  it('imports a Hermes session into the local UI store', async () => {
    const hermesDetail = {
      id: 'cli-1',
      source: 'cli',
      user_id: null,
