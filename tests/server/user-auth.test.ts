...
describe('user auth tables and middleware', () => {
  ...
  it('creates the default super admin without profile bindings', async () => {
    ...
    const created = users.bootstrapDefaultSuperAdmin('admin', 'admin')
    ...
    expect(users.verifyPassword('admin', row.password_hash)).toBe(true)
  ...
  it('authenticates JWTs passed as query tokens for download and websocket URLs', async () => {
    const { users, auth } = await initUsers()
    const user = users.bootstrapDefaultSuperAdmin('admin', 'admin')!
  ...
  it('marks only admin with password admin as requiring a credential change', async () => {
    const { users } = await initUsers()
    const admin = users.bootstrapDefaultSuperAdmin('admin', 'admin')!
    const ctrl = await import('../../packages/server/src/controllers/auth')
    ...
    users.updateUserPassword(admin.id, 'admin')
