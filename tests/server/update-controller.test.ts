...
async function loadUpdateController(overrides: Partial<UpdateControllerMocks> = {}) {
  ...
  const readFileSync = overrides.readFileSync ?? vi.fn(() => JSON.stringify({
    ...
    version: '0.0.0',
    repository: { url: 'https://github.com/Pythoughts-labs/hermes-ui.git' },
  }))
...
function getGlobalCliScript(prefix: string) {
  return process.platform === 'win32'
    ? join(prefix, 'node_modules', '@pythoughts', 'hermes-ui', 'bin', 'hermes-ui.mjs')
    : join(prefix, 'lib', 'node_modules', '@pythoughts', 'hermes-ui', 'bin', 'hermes-ui.mjs')
}
...
describe('update controller', () => {
  ...
  afterEach(() => {
    ...
    delete process.env.HERMES_UI_PREVIEW_REPO
  })
  ...
  it('updates and restarts through the running Node executable, not PATH shims', async () => {
    ...
    expect(mocks.execFile).toHaveBeenCalledWith(
      process.execPath,
      [npmCli, 'install', '-g', '@pythoughts/hermes-ui@latest'],
  ...
  it('keeps update requests responsive while npm install is pending', async () => {
    ...
    const execFile = vi.fn((_command: string, args: string[], _options: any, callback: any) => {
      if (args.includes('install') && args.includes('@pythoughts/hermes-ui@latest')) {
  ...
  it('returns a 500 with stderr when installation fails', async () => {
    ...
    expect(mocks.execFileSync).not.toHaveBeenCalledWith(
      process.execPath,
      [expect.any(String), 'install', '-g', '@pythoughts/hermes-ui@latest'],
      expect.any(Object),
    )
  ...
  it('loads preview tags through async git with a short timeout', async () => {
    process.env.HERMES_UI_PREVIEW_REPO = 'https://github.com/Pythoughts-labs/hermes-ui'
    ...
    expect(mocks.execFile).toHaveBeenCalledWith(
      'git',
      ['ls-remote', '--tags', '--refs', 'https://github.com/Pythoughts-labs/hermes-ui.git'],
  ...
  it('falls back to GitHub API when async git tag loading fails', async () => {
    ...
    expect(fetchMock).toHaveBeenCalledWith(
      'https://api.github.com/repos/Pythoughts-labs/hermes-ui/tags?per_page=100',
      expect.objectContaining({
