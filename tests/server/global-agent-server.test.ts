...
describe('GlobalAgentServer', () => {
  ...
  it('registers a local control namespace with token auth', async () => {
    ...
    const deniedAgent = createMockSocket('socket-denied-agent', { token: 'wrong', role: 'hermes-ui' })
  ...
  it('synthesizes MCU speech at completed assistant-message boundaries', async () => {
    ...
    localSocket.__handlers.get('message.delta')?.({ delta: 'Got it, going to check now.' })
    expect(fetchImpl).not.toHaveBeenCalled()

    localSocket.__handlers.get('tool.started')?.({ tool: 'weather', preview: 'Xiamen weather' })
    await waitForMockCalls(fetchImpl, 1)
    expect(JSON.parse(String(fetchImpl.mock.calls[0][1]?.body))).toMatchObject({
      text: 'Got it, going to check now.',
    })
