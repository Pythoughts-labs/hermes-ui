...
describe('outbound relay client', () => {
  ...
  it('queues the hosted TTS-failed prompt when websocket MCU speech synthesis fails', async () => {
    const fetchImpl = vi.fn(async (url: string) => {
      if (url.includes('/api/hermes/mcu/voice-turn')) {
        return new Response(JSON.stringify({ ok: true, transcript: 'Hello' }), {
          status: 200,
          headers: { 'content-type': 'application/json' },
        })
      }
    ...
    localSocket.__handlers.get('message.delta')?.({ delta: 'Hello' })
    localSocket.__handlers.get('run.completed')?.({})

    await vi.waitFor(() => {
      expect(ws.send).toHaveBeenCalledWith(expect.stringContaining('tts-synthesize-failed.s16le.pcm'))
    })
    ...
    const enqueuePayload = JSON.parse(
      ws.send.mock.calls
        .map(([payload]: [string]) => payload)
        .find((payload: string) => payload.includes('tts-synthesize-failed.s16le.pcm')),
    )
    expect(enqueuePayload).toMatchObject({
      type: 'audio.enqueue',
      interactionId: 'voice-tts-fail',
      segmentId: 'voice-tts-fail-tts-1-failed-prompt',
      text: 'Text-to-speech synthesis failed. Configure a text-to-speech provider in Settings before retrying.',
      url: '/audio/tts-synthesize-failed.s16le.pcm',
      mimeType: 'audio/x-pcm',
      format: 's16le',
      channels: 1,
      sampleRate: 16000,
  ...
  it('forwards an allowed HTTP request to the local UI server', async () => {
