...
describe('shutdown bridge policy', () => {
  ...
  const originalGatewayValue = process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN
  const originalNodeEnv = process.env.NODE_ENV
  const originalDesktop = process.env.HERMES_DESKTOP
  const originalForceExitMs = process.env.HERMES_UI_SHUTDOWN_FORCE_EXIT_MS
  ...
  afterEach(() => {
    ...
    if (originalGatewayValue === undefined) delete process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN
    else process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN = originalGatewayValue
    ...
    if (originalForceExitMs === undefined) delete process.env.HERMES_UI_SHUTDOWN_FORCE_EXIT_MS
    else process.env.HERMES_UI_SHUTDOWN_FORCE_EXIT_MS = originalForceExitMs
  })
  ...
  it('stops managed gateways by default in production only', () => {
    delete process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN
  ...
  it('allows operators to force either managed gateway shutdown policy', () => {
    process.env.NODE_ENV = 'development'
    process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN = '1'
    expect(shouldStopManagedGatewaysOnShutdown()).toBe(true)

    process.env.NODE_ENV = 'production'
    process.env.HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN = '0'
    expect(shouldStopManagedGatewaysOnShutdown()).toBe(false)
  })

  it('keeps desktop shutdown force-exit timing long enough for runtime cleanup by default', () => {
    delete process.env.HERMES_UI_SHUTDOWN_FORCE_EXIT_MS
  ...
  it('allows operators to override shutdown force-exit timing', () => {
    process.env.HERMES_DESKTOP = 'true'
    process.env.HERMES_UI_SHUTDOWN_FORCE_EXIT_MS = '7000'
