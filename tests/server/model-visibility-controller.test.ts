...
describe('models controller — model visibility', () => {
  ...
  it('merges UI custom models into available provider groups', async () => {
    mockReadAppConfig.mockResolvedValue({
      customModels: {
  ...
  it('marks custom-prefixed providers as builtin when their provider key matches a preset', async () => {
    mockReadConfigYamlForProfile.mockResolvedValue({
      model: { default: 'gpt-5.5', provider: 'custom:openai-codex' },
      custom_providers: [
        {
          name: 'openai-codex',
  ...
  it('saves include visibility in ui app config only', async () => {
  ...
  it('resets a provider to all models by deleting its ui visibility rule', async () => {
    mockReadAppConfig.mockResolvedValue({
      modelVisibility: {
  ...
  it('adds and removes custom models in ui app config only', async () => {
