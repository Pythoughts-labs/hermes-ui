...
async function loadCli(overrides: Partial<ChildProcessMocks> = {}) {
  ...
  const mod = await import('../../bin/hermes-ui.mjs')
  return {
    ...mod,
...
describe('CLI port detection', () => {
  ...
  it('clears the login lock file from the configured UI home', async () => {
    const home = mkdtempSync(join(tmpdir(), 'hermes-ui-cli-locks-'))
  ...
  it('cleans a stale server PID file during stop', async () => {
    const home = mkdtempSync(join(tmpdir(), 'hermes-ui-cli-stale-pid-'))
    process.env.HERMES_UI_HOME = home
  ...
  it('allows CLI stop and restart grace periods to be overridden separately', async () => {
    process.env.HERMES_UI_RESTART_GRACE_MS = '2500'
    process.env.HERMES_UI_STOP_GRACE_MS = '9000'
    const { getDaemonStopGraceMs } = await loadCli()

    expect(getDaemonStopGraceMs({ restart: true })).toBe(2_500)
    expect(getDaemonStopGraceMs()).toBe(9_000)

    delete process.env.HERMES_UI_RESTART_GRACE_MS
  ...
  it('skips browser launch when --no-open is provided', async () => {
    ...
    expect(shouldOpenBrowser(['node', 'bin/hermes-ui.mjs', 'start'])).toBe(true)
    expect(shouldOpenBrowser(['node', 'bin/hermes-ui.mjs', 'start', '--no-open'])).toBe(false)
    expect(shouldOpenBrowser(['node', 'bin/hermes-ui.mjs', 'restart', '--port', '9000', '--no-open'])).toBe(false)
  })

  it('preserves --no-open when update respawns restart', async () => {
    const { getRestartArgs } = await loadCli()

    expect(getRestartArgs(9000, ['node', 'bin/hermes-ui.mjs', 'update', '--port', '9000'])).toEqual([
      'restart',
      '--port',
      '9000',
    ])
    expect(getRestartArgs(9000, ['node', 'bin/hermes-ui.mjs', 'update', '--port', '9000', '--no-open'])).toEqual([
      'restart',
      '--port',
      '9000',
      '--no-open',
  ...
  it('resets an existing admin user to the default password', async () => {
    const home = mkdtempSync(join(tmpdir(), 'hermes-ui-cli-default-login-'))
    process.env.HERMES_UI_HOME = home
    const dbPath = join(home, 'hermes-ui.db')
    ...
    try {
      ...
      try {
        ...
        expect(verifyPassword('admin', initial.password_hash)).toBe(true)
      ...
      try {
        ...
        expect(verifyPassword('admin', rows[0].password_hash)).toBe(true)
