...
async function loadController() {
  ...
  process.env.HERMES_UI_HOME = hermesHome
...
describe('config controller locked file updates', () => {
  ...
  it('reads and writes gateway auto-start policy from UI app config', async () => {
    await writeFile(join(hermesHome, 'config.yaml'), [
      'model:',
      '  default: keep-model',
      '',
  ...
  it('does not reconcile gateway management when UI gateway auto-start is disabled', async () => {
