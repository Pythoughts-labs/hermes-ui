...
describe('chat-run message flush', () => {
  ...
  it('flushes tool calls with correct interleaved order', () => {
    ...
    applyToolOutput(state, SID, MARKER, 'call_1', '{"output": "search results returned"}')
  ...
  it('handles text → tool → text without output_text.done between them', () => {
    ...
    expect(assistantTextCalls).toHaveLength(2)
    expect(assistantTextCalls[0][0].content).toBe('Not stuck, just finished searching.')
    expect(assistantTextCalls[1][0].content).toBe('Found it! Details below:')
  })
