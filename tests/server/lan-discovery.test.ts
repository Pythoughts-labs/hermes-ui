...
describe('LAN discovery', () => {
  ...
  it('exposes an MCP terminal list tool so agents can recover forgotten terminal ids', () => {
    const mcpSource = readFileSync('bin/hermes-ui-mcp.mjs', 'utf8')

    expect(mcpSource).toContain("name: 'hermes_ui_lan_devices_list'")
    ...
    expect(mcpSource).toContain("name: 'hermes_ui_lan_terminal_list'")
