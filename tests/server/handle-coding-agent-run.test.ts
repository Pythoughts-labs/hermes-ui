...
describe('handleCodingAgentRun', () => {
  ...
  it('keeps profile token handling separate from the system prompt for authenticated users', async () => {
    ...
    getSystemPromptMock.mockReturnValue([
      'system prompt',
      'Hermes UI MCP usage: call hermes_ui_api_openapi_get before calling unfamiliar UI endpoints.',
      'Use hermes_ui_api_request with method, relative path, and JSON body/query fields.',
    ].join('\n'))
    ...
    expect(prompt).toContain('hermes_ui_api_request')
    expect(prompt).not.toContain('run-token')
    expect(prompt).not.toContain('[Current Hermes profile:')
    expect(prompt).not.toContain('Current Hermes UI model run token')
    expect(prompt).not.toContain('Hermes UI LAN device capabilities are MCP tools')
    expect(prompt).not.toContain('list_mcp_resources')
    expect(prompt).not.toContain('mcp__hermes-ui__')
  })
})
