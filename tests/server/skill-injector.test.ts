...
const originalSkillsDir = process.env.HERMES_UI_SKILLS_DIR
...
async function readManifest(skillsDir: string) {
  return JSON.parse(await readFile(join(skillsDir, '.ui-managed-skills.json'), 'utf-8'))
}
...
afterEach(async () => {
  ...
  if (originalSkillsDir === undefined) delete process.env.HERMES_UI_SKILLS_DIR
  else process.env.HERMES_UI_SKILLS_DIR = originalSkillsDir
...
describe('HermesSkillInjector', () => {
  it('resolves source directories for override, production bundle, and development layouts', async () => {
    ...
    expect(HermesSkillInjector.resolveSourceDir({ HERMES_UI_SKILLS_DIR: override } as any, join(root, 'dist', 'server'))).toBe(override)
  ...
  it('injects missing skills but skips existing user-owned skills with the same name', async () => {
    ...
    await expect(readManifest(join(hermesHome, 'skills'))).resolves.toMatchObject({ 'new-skill': { owner: 'hermes-ui' } })
  ...
  it('updates existing UI-managed bundled copies', async () => {
    const sourceV1 = await tempDir('hermes-skill-source-v1-')
    const sourceV2 = await tempDir('hermes-skill-source-v2-')
    const hermesHome = await tempDir('hermes-skill-home-')
    process.env.HERMES_HOME = hermesHome

    await mkdir(join(sourceV1, 'ui-skill'), { recursive: true })
    await writeFile(join(sourceV1, 'ui-skill', 'SKILL.md'), '# UI Skill v1\n', 'utf-8')
    await mkdir(join(sourceV2, 'ui-skill'), { recursive: true })
    await writeFile(join(sourceV2, 'ui-skill', 'SKILL.md'), '# UI Skill v2\n', 'utf-8')
    ...
    expect(result.updated).toEqual(['ui-skill'])
    expect(result.skipped).toEqual([])
    await expect(readFile(join(hermesHome, 'skills', 'ui-skill', 'SKILL.md'), 'utf-8')).resolves.toBe('# UI Skill v2\n')
    await expect(readManifest(join(hermesHome, 'skills'))).resolves.toMatchObject({ 'ui-skill': { owner: 'hermes-ui' } })
  })
  ...
  it('ignores common OS metadata files when deciding whether a managed copy can update', async () => {
    ...
    const { HermesSkillInjector } = await import('../../packages/server/src/services/hermes/skill-injector')
    await new HermesSkillInjector(sourceV1).injectMissingSkills()
    await writeFile(join(hermesHome, 'skills', 'ui-skill', '.DS_Store'), 'finder metadata', 'utf-8')
    const result = await new HermesSkillInjector(sourceV2).injectMissingSkills()
  ...
  it('adopts identical existing bundled copies without overwriting local files', async () => {
    ...
    await mkdir(join(source, 'ui-skill'), { recursive: true })
    await writeFile(join(source, 'ui-skill', 'SKILL.md'), '# UI Skill\n', 'utf-8')
    await mkdir(join(hermesHome, 'skills', 'ui-skill'), { recursive: true })
    await writeFile(join(hermesHome, 'skills', 'ui-skill', 'SKILL.md'), '# UI Skill\n', 'utf-8')
    ...
    expect(result.updated).toEqual(['ui-skill'])
    expect(result.skipped).toEqual([])
    await expect(readFile(join(hermesHome, 'skills', 'ui-skill', 'SKILL.md'), 'utf-8')).resolves.toBe('# UI Skill\n')
    await expect(readManifest(join(hermesHome, 'skills'))).resolves.toMatchObject({ 'ui-skill': { owner: 'hermes-ui' } })
  })
  ...
  it('syncs bundled skills into default and named profiles without overwriting user-owned conflicts', async () => {
    ...
    await mkdir(join(hermesHome, 'skills', 'ui-skill'), { recursive: true })
    await writeFile(join(hermesHome, 'skills', 'ui-skill', 'SKILL.md'), '# User UI Skill\n', 'utf-8')
    ...
    await mkdir(join(hermesHome, 'profiles', 'beta', 'skills', 'ui-skill'), { recursive: true })
    await writeFile(join(hermesHome, 'profiles', 'beta', 'skills', 'ui-skill', 'SKILL.md'), '# Old Profile Skill\n', 'utf-8')
    ...
    expect(result.injected).toEqual(['ui-skill'])
    expect(result.updated).toEqual([])
    expect(result.skipped).toEqual(['ui-skill', 'ui-skill'])

    await expect(readFile(join(hermesHome, 'skills', 'ui-skill', 'SKILL.md'), 'utf-8')).resolves.toBe('# User UI Skill\n')
    await expect(readFile(join(hermesHome, 'profiles', 'alpha', 'skills', 'ui-skill', 'SKILL.md'), 'utf-8')).resolves.toBe('# UI Skill\n')
    await expect(readFile(join(hermesHome, 'profiles', 'beta', 'skills', 'ui-skill', 'SKILL.md'), 'utf-8')).resolves.toBe('# Old Profile Skill\n')
