...
describe('coding agent Windows process launch', () => {
  it('runs npm .cmd shims through cmd.exe for hidden Claude Code chat turns', () => {
    ...
    manager.start({
      ...
      args: ['--settings', 'C:\\Users\\Administrator\\.hermes-ui\\settings.json'],
      shellCommand: 'claude',
  ...
  it('includes decoded stderr detail when a hidden Codex process exits non-zero', () => {
    ...
    testState.spawnCalls[0].child.stderr.emit('data', Buffer.from('Permission denied', 'utf-8'))
    testState.spawnCalls[0].child.emit('exit', 1)

    expect(emitted).toContainEqual(expect.objectContaining({
      event: 'run.failed',
      payload: expect.objectContaining({
        error: 'Codex exited with code 1: Permission denied',
      }),
    }))
