...
function makeHome() {
  ...
  homes.push(home)
  process.env.HERMES_UI_HOME = home
...
afterEach(() => {
  delete process.env.HERMES_UI_HOME
...
describe('coding agent launch preparation', () => {
  it('launches Claude Code with the global config when requested', async () => {
    ...
    expect(prompt).toContain('<!-- BEGIN HERMES UI PROMPT -->')
    expect(prompt).toContain('# Output Format Specification')
  })
  ...
  it('preserves existing global Claude Code prompt files while updating the Hermes block', async () => {
    ...
    expect(claudePrompt.match(/BEGIN HERMES UI PROMPT/g)).toHaveLength(1)
  })
  ...
  it('launches Claude Code with scoped settings instead of a CLI --model override', async () => {
    ...
    expect(mcp.mcpServers['hermes-ui-devices']).toMatchObject({
      command: process.execPath,
    ...
    expect(mcp.mcpServers['hermes-ui-use']).toMatchObject({
      command: process.execPath,
    ...
    expect(prompt).toContain('# Output Format Specification')
    expect(prompt).toContain('When your reply contains images, videos, or file references')
  })
  ...
  it('cleans legacy Hermes MCP entries from scoped Claude and Codex configs', async () => {
    ...
    expect(claudeMcp.mcpServers['hermes-ui']).toBeUndefined()
    expect(claudeMcp.mcpServers['hermes-ui-mcp']).toBeUndefined()
    expect(claudeMcp.mcpServers.custom).toEqual({ command: 'custom-mcp' })
    expect(claudeMcp.mcpServers['hermes-ui-api']).toBeDefined()
    expect(claudeMcp.mcpServers['hermes-ui-devices']).toBeDefined()
    expect(claudeMcp.mcpServers['hermes-ui-use']).toBeDefined()
    ...
    writeFileSync(codexConfigPath, [
      '[mcp_servers.hermes-ui]',
      'command = "hermes-ui-mcp"',
      '[mcp_servers.hermes-ui-mcp]',
      'command = "hermes-ui-mcp"',
      '',
    ].join('\n'))
    ...
    expect(codexConfig).not.toContain('[mcp_servers.hermes-ui]')
    expect(codexConfig).not.toContain('[mcp_servers.hermes-ui-mcp]')
    expect(codexConfig).toContain('[mcp_servers.hermes-ui-api]')
    expect(codexConfig).toContain('[mcp_servers.hermes-ui-devices]')
    expect(codexConfig).toContain('[mcp_servers.hermes-ui-use]')
  })
  ...
  it('keeps Codex model selection on the CLI while isolating CODEX_HOME', async () => {
    ...
    expect(config).toContain('Hermes UI MCP usage')
    expect(config).toContain('# Output Format Specification')
    expect(config).toContain('[mcp_servers.hermes-ui-api]')
    expect(config).toContain('[mcp_servers.hermes-ui-devices]')
    expect(config).toContain('[mcp_servers.hermes-ui-use]')
    expect(config).toContain(`command = "${process.execPath}"`)
    expect(config).toContain(`args = ["${join(process.cwd(), 'bin/hermes-ui-mcp.mjs')}", "api"]`)
    expect(config).toContain(`args = ["${join(process.cwd(), 'bin/hermes-ui-mcp.mjs')}", "devices"]`)
    expect(config).toContain(`args = ["${join(process.cwd(), 'bin/hermes-ui-mcp.mjs')}", "use"]`)
    expect(config).toContain(`env = { HERMES_UI_URL = "http://127.0.0.1:8648", HERMES_UI_HOME = "${home}"`)
    expect(config).toContain('HERMES_UI_STATE_DIR = "')
    expect(config).toContain('HERMES_UI_PROFILE = "default"')
    expect(config).toContain('HERMES_MCP_SERVER_NAME = "hermes-ui-api"')
    expect(config).toContain('HERMES_MCP_SERVER_NAME = "hermes-ui-devices"')
    expect(config).toContain('HERMES_MCP_SERVER_NAME = "hermes-ui-use"')
    ...
    expect(config).toContain('HERMES_UI_MANAGED_MCP = "1"')
