...
describe('Profile Routes', () => {
  ...
  describe('profile avatars', () => {
    it('stores generated avatar metadata under the UI home', async () => {
      const uiHome = await mkdtemp(join(tmpdir(), 'hermes-ui-avatar-'))
      tempHomes.push(uiHome)
      process.env.HERMES_UI_HOME = uiHome
      ...
      const metaPath = join(uiHome, 'profile-metadata', Buffer.from('work', 'utf-8').toString('base64url'), 'avatar.json')
      expect(ctx.status).toBe(200)
    ...
    it('stores uploaded image avatars and returns a data URL', async () => {
      ...
      const dir = join(uiHome, 'profile-metadata', Buffer.from('work', 'utf-8').toString('base64url'))
    ...
    it('deletes profile avatar metadata', async () => {
      ...
      tempHomes.push(uiHome)
      process.env.HERMES_UI_HOME = uiHome
      const metadataDir = join(uiHome, 'profile-metadata', Buffer.from('work', 'utf-8').toString('base64url'))
