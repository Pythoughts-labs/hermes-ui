...
describe('agent runner endpoint resolver', () => {
  ...
  it('does not duplicate existing OpenAI-compatible API roots', () => {
    ...
    expect(responsesUrl('https://api.example.com/v1/')).toBe('https://api.example.com/v1/responses')
  })
  ...
  it('handles Anthropic-compatible roots', () => {
    expect(anthropicMessagesUrl('https://api.example.com')).toBe('https://api.example.com/v1/messages')
...
describe('coding agent run state', () => {
  it('marks existing scoped Codex runners incompatible when Hermes MCP config is missing', () => {
    ...
    try {
      ...
      writeFileSync(join(codexHome, 'config.toml'), '[mcp_servers.hermes-ui]\ncommand = "node"\n')
      expect(manager.isSessionLaunchCompatible('chat-session-1', {
        agentId: 'codex',
        mode: 'scoped',
        provider: 'test-provider',
  ...
  it('does not duplicate replayed Codex assistant message text', () => {
    ...
    const text = 'OK, I want to view the Meme Rank API. Let me read its details first.'
    ;(manager as any).handleCodexExecLine(run, JSON.stringify({
      method: 'item/agentMessage/delta',
      params: { delta: text },
    }))
  ...
  it('waits for Codex process exit before flushing final text after tools', () => {
    ...
    const openingText = 'I will first check your desktop directory.'
    ;(manager as any).handleCodexExecLine(run, JSON.stringify({
      type: 'item.completed',
    ...
    const finalText = 'Your desktop has the following 3 directories: ai-assets, cache, git.'
    ;(manager as any).handleCodexExecLine(run, JSON.stringify({
      type: 'item.completed',
