...
describe('hermes-ui MCP server', () => {
  ...
  it('exposes a public UI API requester tool', async () => {
    const home = mkdtempSync(join(tmpdir(), 'hermes-ui-mcp-'))
    homes.push(home)
    ...
    child = spawn(process.execPath, ['bin/hermes-ui-mcp.mjs'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        HERMES_UI_URL: `http://127.0.0.1:${address.port}`,
        HERMES_UI_HOME: home,
        HERMES_UI_PROFILE: 'research',
      },
    })
    ...
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_api_request')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_lan_devices_list')).toBe(false)
  ...
  it.each([
    ...
    expect(code).toBe(0)
    expect(stdout.trim()).toBe(`hermes-ui-mcp v${pkg.version}`)
  })

  it('exposes curated Hermes UI use tools in the use toolset', async () => {
    const server = createServer((req, res) => {
      res.setHeader('content-type', 'application/json')
      if (req.url === '/api/chat-run/runs') {
        let raw = ''
    ...
    const responses = new Map<number, any>()
    child = spawn(process.execPath, ['bin/hermes-ui-mcp.mjs', 'use'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        HERMES_UI_URL: `http://127.0.0.1:${address.port}`,
    ...
    const list = await waitForRpc(responses, 2)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_chat_run')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_sessions_count')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_usage_stats')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_session_context')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_session_delete')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_session_rename')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_model_provider_get')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_provider_add')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_provider_delete')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_worker_status')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_workflows_list')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_use_workflow_run_rerun_from_node')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_api_request')).toBe(false)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_lan_devices_list')).toBe(false)
  ...
  it('filters exposed tools by MCP toolset argument', async () => {
    ...
    const responses = new Map<number, any>()
    child = spawn(process.execPath, ['bin/hermes-ui-mcp.mjs', 'devices'], {
      cwd: process.cwd(),
      env: {
        ...process.env,
        HERMES_UI_URL: `http://127.0.0.1:${address.port}`,
    ...
    const list = await waitForRpc(responses, 2)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_lan_devices_list')).toBe(true)
    expect(list.result.tools.some((tool: any) => tool.name === 'hermes_ui_api_request')).toBe(false)
