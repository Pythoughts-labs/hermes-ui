...
// This loaded bundle's baked version. Needs to be a real semver (not 'test')
// so the strictly-newer comparison in clientOutdated is meaningful, and a
// middle value so we can place the server above and below it. Hoisted to run
// before the store module captures UI_VERSION from __APP_VERSION__.
vi.hoisted(() => { (globalThis as typeof globalThis & { __APP_VERSION__: string }).__APP_VERSION__ = '0.5.0' })
...
describe('App Store', () => {
  ...
  it('marks the client stale when the server is newer than this loaded bundle', async () => {
    ...
    expect(store.uiVersion).toBe('0.5.0')
    expect(store.serverVersion).toBe('0.5.17')
    expect(store.clientOutdated).toBe(true)
    expect(store.updateAvailable).toBe(false)
  })

  it('does not mark the client stale when the served UI version matches this bundle', async () => {
    mockSystemApi.checkHealth.mockResolvedValue({
      status: 'ok',
      ui_version: '0.5.0',
      ui_latest: '0.5.0',
    ...
    expect(store.serverVersion).toBe('0.5.0')
    expect(store.clientOutdated).toBe(false)
  })

  it('does not prompt a reload when the server is older than this bundle (release race)', async () => {
    // The screenshot bug: client bundle (0.5.0) is ahead of the server (0.4.0)
    // mid-release. A plain `!==` reported the client stale and the reload button
    // never cleared; reloading cannot help when the bundle is already newer.
    mockSystemApi.checkHealth.mockResolvedValue({
      status: 'ok',
      ui_version: '0.4.0',
      ui_latest: '0.4.0',
    ...
    expect(store.serverVersion).toBe('0.4.0')
    expect(store.clientOutdated).toBe(false)
  })

  it('compares versions numerically, not lexicographically', async () => {
    // "0.10.0" sorts before "0.5.0" as a plain string; it must be treated as newer.
    mockSystemApi.checkHealth.mockResolvedValue({
      status: 'ok',
      ui_version: '0.10.0',
      ui_latest: '0.10.0',
  ...
  it('clears the updating state and reports failure when self-update request fails', async () => {
    ...
    expect(consoleError).toHaveBeenCalledWith('Failed to update Hermes UI:', expect.any(Error))
  ...
  it('saves and clears model aliases via the UI-only alias API', async () => {
