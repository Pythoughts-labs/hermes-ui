...
describe('AppSidebar navigation', () => {
  ...
  it('shows the loaded bundle version in the footer, not the server version', () => {
    // Regression: the footer must report this loaded bundle (uiVersion). It used to
    // show serverVersion, so when the reload prompt fired the footer and the
    // "Reload for v{server}" button showed the identical number.
    mockAppStore.clientOutdated = true
    mockAppStore.uiVersion = '0.1.0'
    mockAppStore.serverVersion = '0.2.0'
    const wrapper = mount(AppSidebar, {
      global: {
        stubs: {
          ProfileSelector: true,
    ...
    expect(wrapper.find('.sidebar-footer__version-text').text()).toBe('Build v0.1.0')
  })
  ...
  it('uses short group labels and keeps group folding active when collapsed', async () => {
    ...
    expect(wrapper.findAll('.sidebar-group__label-text').map(node => node.text())).toEqual([
    ...
    const agentGroup = wrapper.findAll('.sidebar-group')[0]
    expect(agentGroup.find('.sidebar-group__items').attributes('style')).toBeUndefined()

    await agentGroup.find('.sidebar-group__label').trigger('click')
    expect(agentGroup.find('.sidebar-group__items').attributes('style')).toContain('display: none')
  })
