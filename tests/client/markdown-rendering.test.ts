...
describe('MarkdownRenderer', () => {
  ...
  it('renders outer markdown draft fences as markdown while preserving nested fenced examples', () => {
    const wrapper = mount(MarkdownRenderer, {
      props: {
        content: [
          'Below is a PR draft you can directly edit.',
          '',
          '```md',
          'Title: fix(chat): keep attachments in the context of subsequent turns in the same chat',
          '',
          '## Summary',
          '',
          'After the attachment upload, the first turn `startRun()` `input` already includes the uploaded file reference:',
          '',
          '```md',
          '[File: screenshot.png](/uploaded/path)',
          '```',
          '',
          'But the locally saved user message only retains the UI-visible text.',
          '',
          '## Fix',
          '- Preserve context.',
          '```',
    ...
    expect(wrapper.find('.markdown-body').text()).toContain('But the locally saved user message only retains the UI-visible text.')
  ...
  it('keeps markdown examples with their own nested fences intact after unwrapping a draft fence', () => {
    const wrapper = mount(MarkdownRenderer, {
      props: {
        content: [
          ...
          '## Fix',
          '',
          '- Subsequent heading should not be truncated.',
          '```',
          '',
          '## Local Verification',
          '',
    ...
    expect(codeText).toContain('Below is a PR draft.')
    expect(codeText).toContain('```md\n[File: Screenshot.png](/tmp/example.png)\n```')
    expect(codeText).toContain('## Fix')
    expect(codeText).toContain('- Subsequent heading should not be truncated.')
  ...
  it('renders local mov links as inline video players', () => {
    const wrapper = mount(MarkdownRenderer, {
      props: {
        content: '[Recording-2026-05-08 15.19.46.mov](/Users/elkaix/Desktop/Recording-2026-05-08%2015.19.46.mov)',
      },
    })
    ...
    expect(decodeURIComponent(src.searchParams.get('path') || '')).toBe('/Users/elkaix/Desktop/Recording-2026-05-08 15.19.46.mov')
    expect(wrapper.find('.markdown-video-footer .att-name').text()).toBe('Recording-2026-05-08 15.19.46.mov')
  })
  ...
  it('renders MSYS-style Windows image paths through the download endpoint', () => {
    const wrapper = mount(MarkdownRenderer, {
      props: {
        content: '![Desktop screenshot](/c/Users/Administrator/Desktop/screenshot.png)',
      },
    })
    ...
    expect(img.attributes('alt')).toBe('Desktop screenshot')
  })
