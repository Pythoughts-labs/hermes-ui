...
const mockFetchLocalTtsStatus = vi.fn().mockResolvedValue({ model: { status: 'ready' } })
const mockStartLocalTtsDownload = vi.fn().mockResolvedValue({ ok: true })
const mockRemoveLocalTtsModel = vi.fn().mockResolvedValue({ ok: true })
...
const translations: Record<string, string> = {
  ...
  'settings.voice.presetLocalTtsLabel': 'Local TTS',
  'settings.voice.presetLocalTtsDescription': 'On-device speech synthesis powered by Kokoro-82M.',
  ...
  'settings.voice.providerEdge': 'Local TTS (Free, on-device)',
  ...
  'settings.voice.sttFailureHintNetworkCustom': 'The server could not reach the custom STT endpoint. Check that {endpoint} is reachable from the UI server and is OpenAI-compatible.',
  'settings.voice.sttFailureHintNetwork': 'The UI server could not reach the STT provider. Check network/proxy/firewall and retry.',
...
vi.mock('@/api/hermes/tts-settings', () => ({
  ...
  fetchLocalTtsStatus: mockFetchLocalTtsStatus,
  startLocalTtsDownload: mockStartLocalTtsDownload,
  removeLocalTtsModel: mockRemoveLocalTtsModel,
}))
...
function createVoiceSettingsMock() {
  return {
    ...
    localVoice: ref('af_heart'),
    localSpeed: ref(1.0),
    setLocalVoice(value: string) { this.localVoice.value = value },
    setLocalSpeed(value: number) { this.localSpeed.value = value },
...
describe('VoiceSettings STT UI', () => {
  ...
  it('renders card-based TTS/STT voice APIs with built-ins and masked stored secrets', async () => {
    ...
    mockFetchTtsSettings.mockResolvedValue({
      providers: [
        {
          provider: 'mimo',
          settings: { baseUrl: 'https://api.xiaomimimo.com/v1', model: 'mimo-v2.5-tts', voice: 'rock-candy' },
          secrets: { apiKey: '[stored]' },
          updatedAt: 4,
        },
      ],
    ...
    expect(wrapper.text()).toContain('Local TTS')
  ...
  it('shows missing-key and inline testing/error states on provider cards', async () => {
    ...
    await wrapper.get('[data-testid="voice-card-test-tts-local"]').trigger('click')
    await nextTick()
