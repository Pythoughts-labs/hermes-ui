...
describe('client TTS unified synthesize flow', () => {
  ...
  it('openaiPlay respects an explicit local provider even when other fields look OpenAI-compatible', async () => {
    mockFetch.mockResolvedValue(new Response(new Blob(['local-audio'], { type: 'audio/wav' }), {
      status: 200,
      headers: { 'X-TTS-Provider': 'local' },
    }))
    const { useSpeech } = await import('../../packages/client/src/composables/useSpeech')
    const speech = useSpeech()

    await speech.openaiPlay('msg-local-explicit', 'Local explicit', {
      provider: 'local',
      baseUrl: 'https://api.openai.com/v1',
      model: 'tts-1',
      voice: 'af_heart',
    ...
    expect(JSON.parse(options.body).provider).toBe('local')
  })

  it('openaiPlay falls back to local for the legacy /api/tts/proxy baseUrl', async () => {
    mockFetch.mockResolvedValue(new Response(new Blob(['local-audio'], { type: 'audio/wav' }), {
      status: 200,
      headers: { 'X-TTS-Provider': 'local' },
    }))
    ...
    await speech.openaiPlay('msg-local-fallback', 'Local fallback', {
...
describe('client TTS autoplay call sites', () => {
  ...
  it('does not require a local MiMo API key before using server-stored TTS credentials', () => {
    ...
    expect(messageItem).not.toContain('MiMo TTS API Key is empty')
