...
describe('desktop runtime manager', () => {
  beforeEach(() => {
    ...
    process.env.HERMES_UI_HOME = tempDir('hermes-runtime-home-')
  ...
  it('downloads through a unique temp file instead of reusing a stale runtime .download path', async () => {
    ...
    expect(existsSync(join(process.env.HERMES_UI_HOME!, 'desktop-runtime', 'active-version.json'))).toBe(true)
  })
})
