...
describe('desktop runtime paths', () => {
  beforeEach(() => {
    ...
    process.env.HERMES_UI_HOME = tempDir()
  ...
  it('uses active-version.json paths for startup while keeping the current target runtime path', async () => {
    ...
    const expectedUiDir = tempDir()
    process.env.HERMES_UI_HOME = homeDir
    ...
    writeFileSync(join(homeDir, 'desktop-runtime', 'active-version.json'), JSON.stringify({
      ...
      uiDirectory: expectedUiDir,
      platform: runtimePlatformKey(),
    }))

    const { desktopRuntimeDir, targetDesktopRuntimeDir, uiDir } = await import('../../packages/desktop/src/main/paths')

    expect(desktopRuntimeDir()).toBe(runtimeDir)
    expect(uiDir()).toBe(expectedUiDir)
    expect(targetDesktopRuntimeDir()).toBe(join(homeDir, 'desktop-runtime', 'hermes', '0.15.2', runtimePlatformKey()))
  })

  it('removes downloaded UI caches below 0.6.14 so startup falls back to the bundled UI', async () => {
    const homeDir = tempDir()
    const appPath = tempDir()
    const legacyUiDir = join(homeDir, 'ui', '0.6.13')
    const currentUiDir = join(homeDir, 'ui', '0.6.14')
    process.env.HERMES_UI_HOME = homeDir
    mockElectronApp.getAppPath = () => appPath

    const { runtimePlatformKey } = await import('../../packages/desktop/src/main/runtime-paths')
    mkdirSync(join(legacyUiDir, 'dist', 'server'), { recursive: true })
    writeFileSync(join(legacyUiDir, 'package.json'), JSON.stringify({ version: '0.6.13' }))
    writeFileSync(join(legacyUiDir, 'dist', 'server', 'index.js'), '')
    mkdirSync(currentUiDir, { recursive: true })
    writeFileSync(join(currentUiDir, 'package.json'), JSON.stringify({ version: '0.6.14' }))
    mkdirSync(join(homeDir, 'desktop-runtime'), { recursive: true })
    writeFileSync(join(homeDir, 'desktop-runtime', 'active-version.json'), JSON.stringify({
      schema: 1,
      uiVersion: '0.6.13',
      uiDirectory: legacyUiDir,
      platform: runtimePlatformKey(),
    }))

    const { uiDir } = await import('../../packages/desktop/src/main/paths')

    expect(uiDir()).not.toBe(legacyUiDir)
    expect(existsSync(legacyUiDir)).toBe(false)
    expect(existsSync(currentUiDir)).toBe(true)
  })
