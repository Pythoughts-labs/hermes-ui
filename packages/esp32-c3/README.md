...
2. If Wi-Fi is missing or connection fails, it starts the open `HermesUI-WIFI`
   setup hotspot.
3. Join `HermesUI-WIFI` and open `http://192.168.4.1/`.
...
After Wi-Fi is connected, open the device page and use the device tab. The
firmware sends a UDP `hermes.discover` probe to the fixed Hermes discovery port
`48640`. Hermes UI and desktop responders return `hermes.announce` payloads
...
selected profile locally, and connects to the selected UI `/global-agent`
