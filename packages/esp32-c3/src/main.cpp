...
namespace {
constexpr char kApName[] = "HermesUI-WIFI";
...
String mcuSocketStateLabel() {
  if (mcuSocketNamespaceReady) return F("Connected");
  if (mcuSocketConnected) return F("Handshaking");
  if (wsReady) return F("Reconnecting");
...
String endpointLabel(const String &endpointKind, uint16_t httpPort) {
  if (endpointKind == F("web")) return F("UI");
  if (endpointKind == F("desktop")) return F("Desktop");
  return String(F("Custom port ")) + httpPort;
}
...
void rememberLanDeviceInfo(const String &json, const String &host, const String &baseUrl, uint32_t responseMs,
  ...
  device.webVersion = jsonStringValue(json, F("hermes_ui_version"));
...
bool fetchManualDeviceInfo(const String &input, String *error) {
  ...
  if (baseUrl.length() == 0) {
    if (error) *error = F("Invalid device address");
    return false;
  }
  ...
  if (!http.begin(endpoint)) {
    if (error) *error = F("Cannot open device info endpoint");
    return false;
  }
  int code = http.GET();
  ...
  if (jsonStringValue(body, F("device_id")).length() == 0 ||
    ...
    if (error) *error = F("Device info missing device identity");
    return false;
  }
...
void sendWifiPage() {
  ...
  String html = pageStart(F("Connect Wi-Fi"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>Connect to LAN Wi-Fi</h1>");
  if (wifiReady) {
    html += F("<p class='lead ok'>Currently online: ");
    html += escapeHtml(WiFi.SSID());
    html += F(" · IP ");
    html += WiFi.localIP().toString();
    html += F("</p>");
  } else {
    html += F("<p class='lead'>Device is in Wi-Fi setup mode.Connect to the hotspot <strong>");
    html += kApName;
    html += F("</strong> then select or enter your home Wi-Fi below.</p>");
  }
  ...
  if (scannedNetworkCount > 0) {
    ...
    html += F("<option value=''>Select Wi-Fi</option>");
    for (int i = 0; i < scannedNetworkCount; ++i) {
      html += F("<option value='");
      ...
      html += scannedRssi[i];
      html += F(" dBm");
      if (!scannedEncrypted[i]) html += F(" · open");
      html += F("</option>");
    }
    html += F("<option value='__manual__'>Enter SSID manually</option></select></div>");
    html += F("<div class='field'><span class='label'>Manual SSID</span><input name='ssid_manual' autocomplete='off' value='");
    if (!savedInScan) html += escapeHtml(savedSsid);
    html += F("'></div>");
  } else {
    html += F("<p class='hint'>No nearby Wi-Fi found. Please enter the SSID manually.</p>");
  ...
  html += F("<div class='field'><span class='label'>Password</span><input name='pass' type='password' autocomplete='current-password' placeholder='Leave blank for open networks'></div>");
  html += F("<div class='btn-row'><button class='btn primary' type='submit'>Save and connect</button></div>");
  html += F("<p class='hint'>Setup hotspot: ");
  html += kApName;
  html += F(", no password required.Current device URL: http://");
  html += currentIp();
  html += F("/</p></form></section>");

  if (wifiReady || savedSsid.length() > 0) {
    html += F("<form method='post' action='/clear' class='card'><button class='btn primary' type='submit'>Clear Wi-Fi config and return to setup mode</button></form>");
  }
  html += pageEnd();
...
void sendStatusPage() {
  String html = pageStart(F("Device is online"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>Device</h1><p class='lead'>");
  html += escapeHtml(WiFi.SSID());
  html += F(" · IP ");
  html += WiFi.localIP().toString();
  html += F("</p><nav class='tabs'><a class='tab active' href='/device'>Device</a><a class='tab' href='/ota'>OTA</a></nav>");

  html += F("<h2>Local device</h2><div class='info-grid'>");
  ...
  appendInfoRow(html, F("Device code"), mcuDeviceCode());
  appendInfoRow(html, F("Signal"), String(WiFi.RSSI()) + F(" dBm"));
  appendInfoRow(html, F("Uptime"), uptimeText());
  appendInfoRow(html, F("Available memory"), String(ESP.getFreeHeap()) + F(" bytes"));
  appendInfoRow(html, F("Service connection"), mcuSocketStateLabel());
  appendInfoRow(html, F("Audio hardware"), String(es8311Ready && i2sReady ? F("Ready") : F("Not ready")) + F(" · ") + lastAudioDetail);
  if (selectedProfile.length() > 0) {
    appendInfoRow(html, F("Recent Profile"), selectedProfile);
  }
  String activeLabel = activeDeviceLabel();
  if (activeLabel.length() > 0) {
    appendInfoRow(html, F("Current interaction"), activeLabel);
  }
  appendInfoRow(html, F("MCU status"), mcuInteractionStatus);
  appendInfoRow(html, F("Audio queue"), String(mcuAudioPlaying ? F("Playing · ") : F("")) + String(mcuAudioCount));
  html += F("</div>");

  html += F("<div class='btn-row' style='margin-top:16px'><a class='btn primary' href='/device/scan'>Refresh scan</a><a class='btn' href='/wifi'>Reconfigure Wi-Fi</a></div>");
  html += F("</section>");

  html += F("<section class='card'><h2>Add device manually</h2>");
  html += F("<form method='post' action='/device/manual'><div class='field'><span class='label'>Address</span>");
  html += F("<input name='url' autocomplete='off' placeholder='http://192.168.1.10:8648' required></div>");
  html += F("<div class='btn-row'><button class='btn primary' type='submit'>Add</button></div>");
  html += F("<p class='hint'>Will request the peer’s device-info endpoint and save it. Existing devices are deduplicated.</p></form></section>");

  html += F("<section class='card'><h2>LAN devices</h2>");
  if (lastLanDiscoveryAtMs == 0) {
    html += F("<p class='hint'>Not scanned yet. Click “Refresh scan” to broadcast on the fixed discovery port and wait for UI / Desktop responses.</p>");
  } else {
    html += F("<p class='hint'>Last scan: ");
    html += String((millis() - lastLanDiscoveryAtMs) / 1000UL);
    html += F("s ago</p>");
  }

  if (lanDeviceCount == 0) {
    html += F("<p class='lead'>No devices found</p>");
  } else {
    ...
    for (int i = 0; i < lanDeviceCount; ++i) {
      ...
      html += online ? F(" · online") : F(" · May be offline");
      html += device.loggedIn ? F(" · signed in") : F(" · not signed in");
      if (lanDeviceKey(device) == activeDeviceKey) html += F(" · Current interaction");
      html += F("</span><strong>");
      ...
      if (device.loggedIn) {
        if (lanDeviceKey(device) != activeDeviceKey) {
          ...
          html += i;
          html += F("'>Set as current</a>");
        }
        html += F("<a class='btn' href='/device/profile?i=");
        html += i;
        html += F("'>Switch Profile</a>");
...
String otaNextCheckText() {
  ...
  if (remaining <= 0) return F("Checking soon");
  return String(static_cast<uint32_t>(remaining) / 1000UL) + F("s later");
}

void sendOtaPage(const String &notice = "") {
  String html = pageStart(F("OTA"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>OTA</h1><p class='lead'>Firmware update</p>");
  html += F("<nav class='tabs'><a class='tab' href='/device'>Device</a><a class='tab active' href='/ota'>OTA</a></nav>");
  if (notice.length() > 0) {
    html += F("<p class='hint'>");
    html += escapeHtml(notice);
    html += F("</p>");
  }
  html += F("<h2>Firmware status</h2><div class='info-grid'>");
  appendInfoRow(html, F("Current MD5"), ESP.getSketchMD5());
  appendInfoRow(html, F("Server"), activeDeviceUrl.length() > 0 ? activeDeviceUrl : String(F("Not connected")));
  appendInfoRow(html, F("Manifest"), activeDeviceEndpoint(F("/api/hermes/mcu/firmware/manifest")));
  appendInfoRow(html, F("Next auto-check"), otaNextCheckText());
  appendInfoRow(html, F("OTA conditions"), String(wifiReady && WiFi.status() == WL_CONNECTED ? F("Wi-Fi OK") : F("Wi-Fi OFF")) +
  ...
  html += F("<button class='btn primary' type='submit'>Check for updates</button><a class='btn' href='/device'>Back to device</a></form>");
  html += F("<p class='hint'>Auto-update runs when idle. If new firmware is found, the device downloads it, flashes the OTA partition, and reboots.</p>");
  html += F("</section>");
  html += pageEnd();
  server.send(200, F("text/html; charset=utf-8"), html);
}
...
void sendOtaUpdatingPage() {
  String html = pageStart(F("OTA"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>OTA</h1><p class='lead'>Updating firmware</p>");
  html += F("<p class='hint'>Firmware is downloading and flashing. Do not power off the device. It will reboot automatically; the page will detect recovery and prompt when complete.</p>");
  html += F("<div class='info-grid'>");
  appendInfoRow(html, F("Current status"), F("Updating — do not power off the device"));
  appendInfoRow(html, F("Server"), activeDeviceUrl.length() > 0 ? activeDeviceUrl : String(F("Not connected")));
  html += F("</div><p id='ota-status' class='hint'>Waiting for device to reboot...</p>");
  html += F("<script>");
  ...
  html += F("if(r.ok&&seenOffline){done=true;s.textContent='Firmware update complete; device is back online.';alert('Firmware update complete; device has rebooted.');location.href='/ota';return;}");
  html += F("if(r.ok){s.textContent='Writing firmware; do not power off the device...';}");
  html += F("}catch(e){seenOffline=true;s.textContent='Device is rebooting; please wait for recovery...';}");
  html += F("if(Date.now()-start>120000&&!done){done=true;s.textContent='Update timed out. Please reopen the device page to confirm status.';alert('Update status confirmation timed out. Please reopen the device page.');return;}");
...
void handleOtaCheck() {
  ...
  if (result == McuOtaResult::NoUpdate) {
    sendOtaPage(F("Already running the latest firmware, no update needed."));
    return;
  }
  ...
  sendOtaPage(F("Check failed. Confirm the device is logged in and the server was restarted."));
}
...
void addManualDevice() {
  ...
  if (!fetchManualDeviceInfo(url, &error)) {
    String html = pageStart(F("Add failed"));
    html += F("<section class='panel'><p class='meta'>MANUAL DEVICE</p><h1>Add failed</h1><p class='lead bad'>");
    html += escapeHtml(error.length() > 0 ? error : String(F("Cannot fetch device info")));
    html += F("</p><div class='btn-row'><a class='btn primary' href='/device'>Back to device</a></div></section>");
    html += pageEnd();
...
bool runMcuLogin(LanDevice &device, const String &account, const String &password) {
  ...
  http.setTimeout(kMcuLoginTimeoutMs);
  if (!http.begin(endpoint)) {
    rememberMcuLoginResult(-1, F("Cannot open sign-in endpoint"));
  ...
  http.addHeader(F("X-Hermes-Device-Name"), F("Hermes UI ESP32-C3"));
...
void sendMcuLoginPage() {
  ...
  if (!lanDeviceIndex(index, &device)) {
    server.send(404, F("text/plain; charset=utf-8"), F("Device does not exist; refresh scan first"));
    return;
  }

  String html = pageStart(F("Sign inDevice"));
  html += F("<section class='panel'><p class='meta'>MCU LOGIN</p><h1>Sign in ");
  ...
  html += index;
  html += F("'><div class='field'><span class='label'>Account</span><input name='account' autocomplete='username' required></div>");
  html += F("<div class='field'><span class='label'>Password</span><input name='password' type='password' autocomplete='current-password' required></div>");
  html += F("<div class='btn-row'><button class='btn primary' type='submit'>Sign in</button><a class='btn' href='/device'>Back</a></div>");
  html += F("<p class='hint'>After signing in, you can select a profile. The device will use the sign-in token to connect to the current machine.</p></form></section>");
  html += pageEnd();
  server.send(200, F("text/html; charset=utf-8"), html);
}

void sendProfilePage() {
  String html = pageStart(F("Select Profile"));
  html += F("<section class='panel'><p class='meta'>MCU LOGIN</p><h1>Select Profile</h1>");
  if (lastMcuLoginCode >= 200 && lastMcuLoginCode < 300) {
    html += F("<p class='lead ok'>Sign in successful</p>");
  } else {
    html += F("<p class='lead bad'>Sign in failed: ");
  ...
  if (loginProfileCount > 0) {
    ...
    html += F("</select></div><div class='btn-row'><button class='btn primary' type='submit'>Use this Profile</button><a class='btn' href='/device'>Back to device</a></div></form>");
  } else {
    html += F("<p class='hint'>No selectable profile.</p><div class='btn-row'><a class='btn primary' href='/device'>Back to device</a></div>");
  }
  html += pageEnd();
  server.send(200, F("text/html; charset=utf-8"), html);
}
...
void switchProfilePage() {
  ...
  if (loginProfileCount == 0) {
    String html = pageStart(F("Sign in required"));
    html += F("<section class='panel'><p class='meta'>MCU LOGIN</p><h1>Sign in required</h1><p class='lead'>No cached profile list. Please sign in again to switch.</p><div class='btn-row'><a class='btn primary' href='/device/login?i=");
    html += index;
    html += F("'>Sign in again</a><a class='btn' href='/device'>Back to device</a></div></section>");
    html += pageEnd();
    server.send(200, F("text/html; charset=utf-8"), html);
    return;
  }
...
void handleMcuLogin() {
  ...
  if (account.length() == 0 || password.length() == 0) {
    server.send(400, F("text/plain; charset=utf-8"), F("Missing account or password"));
    return;
  }
...
void saveProfile() {
  ...
  String html = pageStart(F("Profile selected"));
  html += F("<section class='panel'><p class='meta'>MCU LOGIN</p><h1>Profile selected</h1><p class='lead ok'>");
...
void enqueueMissingSttPrompt(const String &interactionId) {
  ...
  segment.text = F("The current profile has no speech-to-text configured. Please configure it before using.");
...
void enqueueNoDevicePrompt(const String &interactionId) {
  ...
  segment.text = F("No device is currently connected. Please connect a device from the UI.");
...
void handleVoiceTurnResponse(const String &interactionId, const String &response, int statusCode) {
  ...
  if (audioUrl.length() > 0) {
    ...
    if (segment.text.length() == 0) segment.text = F("Voice prompt");
...
String mcuSocketAuthJson() {
  ...
  json += F("\",\"role\":\"hermes-ui\",\"instanceId\":\"");
...
void sendConnectSuccessPage(const String &ssid, const IPAddress &ip) {
  ...
  String html = pageStart(F("Wi-Fi Connected"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>Wi-Fi Connected</h1>");
  html += F("<p class='lead ok'>");
  html += escapeHtml(ssid);
  html += F(" · IP ");
  html += ip.toString();
  html += F("</p><p class='hint'>The device will reboot, close the setup hotspot, then open the LAN address. If your phone does not auto-reconnect to the same Wi-Fi, please switch back manually before opening the URL.</p>");
  html += F("<div class='btn-row'><a class='btn primary' href='");
  html += target;
  html += F("'>Open device page</a><a class='btn' href='/wifi'>Reconfigure</a></div></section>");
...
void sendConnectFailedPage(const String &ssid) {
  String html = pageStart(F("Wi-Fi connection failed"));
  html += F("<section class='panel'><p class='meta'>Hermes UI ESP32-C3</p><h1>Wi-Fi connection failed</h1>");
  html += F("<p class='lead bad'>Could not connect to ");
  html += escapeHtml(ssid);
  html += F(". Please check the SSID and password, then retry.</p>");
  html += F("<div class='btn-row'><a class='btn primary' href='/wifi'>Back to setup</a><a class='btn primary' href='/clear'>Clear config</a></div></section>");
  html += pageEnd();
  server.send(200, F("text/html; charset=utf-8"), html);
}
...
void saveWifi() {
  ...
  if (ssid.length() == 0) {
    server.send(400, F("text/plain; charset=utf-8"), F("Missing SSID"));
    return;
  }
...
void clearWifi() {
  ...
  server.send(200, F("text/html; charset=utf-8"),
              F("<!doctype html><meta charset='utf-8'><p>Cleared. Device is rebooting back to setup mode...</p>"));
  delay(700);
  ESP.restart();
}
...
void setup() {
  ...
  delay(300);
  Serial.println();
  Serial.println(F("Hermes UI WiFi setup firmware boot"));
  Serial.printf("Reset reason=%d free_heap=%lu min_free_heap=%lu\n",
                static_cast<int>(esp_reset_reason()),
                static_cast<unsigned long>(ESP.getFreeHeap()),
                static_cast<unsigned long>(ESP.getMinFreeHeap()));
  esp_rom_printf("Hermes UI WiFi setup firmware boot\n");
  initOledDisplay();
  initAudioHardware();
  prefs.begin("mcu", true);
