 ...
 *
 * Server/listen:
 * - PORT: UI listen port. Default: 8648.
 * - BIND_HOST: UI bind host. Default: 0.0.0.0.
 * - CORS_ORIGINS: Comma/space-separated cross-origin allowlist. Default: same host only.
 *
 * UI storage:
 * - HERMES_UI_HOME: UI data home for auth token, credentials, logs, DB, and default uploads.
 * - HERMES_UI_STATE_DIR: Compatibility alias for HERMES_UI_HOME.
 *   Default: join(homedir(), '.hermes-ui').
 * - UPLOAD_DIR: Upload directory override. Default: join(HERMES_UI_HOME, 'upload').
 * - dataDir: Development-only internal UI runtime data directory.
 *
 * Auth:
 * - AUTH_TOKEN: Explicit bearer token. If unset, UI stores an auto-generated token under HERMES_UI_HOME.
 *
 * Runtime behavior:
 ...
 * - HERMES_UI_DISABLE_GATEWAY_AUTOSTART: Disable UI gateway autostart checks and config-driven gateway start/stop reconciliation.
 * - HERMES_UI_MANAGED_GATEWAY: UI-managed Hermes gateway handling. Enabled by default; set 0/false/off to use CLI start.
 * - HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN: Whether UI shutdown also stops managed gateways.
 * - HERMES_UI_DISABLE_MCP_AUTOINJECT: Disable Hermes UI MCP config injection.
 * - HERMES_UI_ALLOW_TRANSIENT_MCP_AUTOINJECT: Allow MCP injection when HERMES_UI_HOME is under a temp dir.
...
export function getUiHome(env: Record<string, string | undefined> = process.env): string {
  const appHome = env.HERMES_UI_HOME?.trim() || env.HERMES_UI_STATE_DIR?.trim()
  return appHome ? resolve(appHome) : join(homedir(), '.hermes-ui')
}

export function shouldCreateUiDataDir(env: Record<string, string | undefined> = process.env): boolean {
