...
export type StoredTtsProvider = 'openai' | 'custom' | 'local' | 'mimo' | 'doubao'
...
const PROVIDERS: StoredTtsProvider[] = ['custom', 'doubao', 'local', 'mimo', 'openai']
...
// Migration: legacy builds stored active_provider='edge' and wrote
// tts_settings rows with provider='edge'. Edge was a Microsoft-cloud dependency
// (node-edge-tts) and is replaced by the local Kokoro engine. Map legacy
// 'edge' rows/active to 'local' on read so existing users don't end up with a
// dangling active provider that no provider implements.
function migrateLegacyProvider(value: string | null | undefined): string | null {
...
export function getActiveTtsProvider(profile: string): ActiveTtsProvider | null {
  ...
  const activeProvider = migrateLegacyProvider(row?.active_provider)
...
function readStoredRow(profile: string, provider: StoredTtsProvider): StoredRow | null {
  const db = getDb()
  if (!db) return null
  // Ponytail: the legacy 'edge' rows can be ignored — the local provider has
  // its own config (voice + speed) and reads from disk, not from the settings
  // table. Skipping the migration copy saves a row update per profile on
...
function sanitizeStoredSettings(provider: StoredTtsProvider, input: Record<string, unknown>): TtsStoredSettings {
  ...
  for (const key of SETTINGS_KEYS) {
    ...
    if (typeof rawValue === 'number' && Number.isFinite(rawValue) && key === 'speed') {
