...
export const DEVICES_SCHEMA: Record<string, string> = {
  ...
  hermes_ui_version: "TEXT NOT NULL DEFAULT ''",
...
/**
 * Create a table with the full schema.
 */
function createTable(
  ...
  // Only append the composite primary key when the schema does not already define one
 ...
 * - Table exists: only append safe new columns; never drop, rebuild, or alter
 *   the primary key / column types.
 */
export function syncTable(
  tableName: string,
  schema: Record<string, string>,
  options?: {
    primaryKey?: string  // primary-key definition, e.g. "roomId, agentId" or "id"
    indexes?: Record<string, string>  // index definitions
  }
): void {
  const db = getDb()
  if (!db) return

  // 1. Table does not exist → create it directly
