...
export function securityHeaders(): Middleware {
  return async (ctx, next) => {
    ...
    ctx.set('Content-Security-Policy', [
      ...
      "form-action 'self'",
      // ponytail: privacy-first tightening. The previous policy allowed
      // connect-src to any http(s)/ws(s) origin (exfil via XHR/WS) and
      // img-src to any https origin (third-party tracking pixels). Lock both
      // to same-origin; ws/wss to the same host is already covered by 'self'.
