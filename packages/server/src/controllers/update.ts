...
import { getUiHome } from '../config'

let updateInProgress = false
const NODE_ENVIRONMENT_MISSING_CODE = 'node_environment_missing'

const PREVIEW_DIR_NAME = 'hermes-ui-pereview'
const PREVIEW_HOME_DIR_NAME = 'hermes-ui-pereview-home'
...
const PREVIEW_AGENT_BRIDGE_ENDPOINT_ENV = 'HERMES_UI_PREVIEW_AGENT_BRIDGE_ENDPOINT'
const PREVIEW_AGENT_BRIDGE_TRANSPORT_ENV = 'HERMES_UI_PREVIEW_AGENT_BRIDGE_TRANSPORT'
...
function getPreviewRepoBaseUrl(): string {
  const configured = process.env.HERMES_UI_PREVIEW_REPO?.trim()
...
function getGlobalPackageBin(root: string) {
  return join(root, '@pythoughts', 'hermes-ui', 'bin', 'hermes-ui.mjs')
}
...
function getPreviewDir() {
  return join(getUiHome(), PREVIEW_DIR_NAME)
}

function getPreviewHomeDir() {
  return join(getUiHome(), PREVIEW_HOME_DIR_NAME)
}
...
function assertPreviewPackage() {
  ...
  const pkg = JSON.parse(readFileSync(packagePath, 'utf-8'))
  if (pkg?.name !== '@pythoughts/hermes-ui') {
    throw new Error(`Preview directory is not @pythoughts/hermes-ui: ${getPreviewDir()}`)
  }
}
...
function patchPreviewViteConfig(source: string) {
  let next = source.replace(
    ...
    [
      `const BACKEND_PORT = process.env.HERMES_UI_BACKEND_PORT || '${PREVIEW_BACKEND_PORT}'`,
      'const BACKEND = `http://127.0.0.1:${BACKEND_PORT}`',
    ].join('\n'),
  )
  if (!next.includes('HERMES_UI_FRONTEND_PORT')) {
    next = next.replace(
      /server:\s*\{/,
      `server: {\n    port: Number(process.env.HERMES_UI_FRONTEND_PORT || ${PREVIEW_FRONTEND_PORT}),\n    strictPort: true,`,
    )
  }
  next = next.replace(
...
async function downloadGithubZip(ref: string, targetDir: string, type: 'tag' | 'branch' = 'tag') {
  ...
  const res = await fetch(url, {
    headers: { 'User-Agent': 'hermes-ui-preview' },
...
async function getGlobalCliScriptAsync() {
  ...
  if (!existsSync(cli)) {
    throw new Error(`Updated hermes-ui CLI not found: ${cli}`)
  }
  return cli
}
...
async function runUpdateInstall() {
  ...
  return runNpmAsync(['install', '-g', '@pythoughts/hermes-ui@latest'], { timeout: 10 * 60 * 1000 })
}
...
export async function handleUpdate(ctx: any) {
  if (updateInProgress) {
    ...
    ctx.body = {
      success: false,
      message: 'hermes-ui update is already in progress',
    }
    return
  }
  ...
  try {
    ...
    ctx.body = {
      success: true,
      message: output.trim() || 'hermes-ui updated successfully',
    }
  ...
  try {
    ...
    const queued = queuePreviewAction('start', async () => {
      ...
      const env = {
        ...
        HERMES_UI_HOME: getPreviewHomeDir(),
        HERMES_UI_STATE_DIR: getPreviewHomeDir(),
        ...
        AUTH_TOKEN: '',
        HERMES_UI_BACKEND_PORT: String(PREVIEW_BACKEND_PORT),
        HERMES_UI_FRONTEND_PORT: String(PREVIEW_FRONTEND_PORT),
