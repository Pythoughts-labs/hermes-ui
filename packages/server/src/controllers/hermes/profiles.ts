...
function profileMetadataRoot(): string {
  return join(getUiHome(), 'profile-metadata')
}
...
export async function list(ctx: any) {
  try {
    ...
    // UI active profile is request-scoped and comes from X-Hermes-Profile.
    profiles.forEach(p => {
...
export async function create(ctx: any) {
  ...
  try {
    ...
    // When clone=true, run smart cleanup:
    //   - Strip exclusive-platform credentials from .env (Weixin / Telegram / Slack / ...)
    //   - Disable the matching platform nodes in config.yaml
    // Prevents the new profile from sharing a bot token with the source profile,
    // which would trigger mutex conflicts.
    ...
    if (clone) {
      try {
        ...
        // Cleanup failures must not block profile creation — just log them
