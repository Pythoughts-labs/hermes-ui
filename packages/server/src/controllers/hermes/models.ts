...
function filterModelsForProvider(provider: string, models: string[], visibility: ModelVisibility): string[] {
  ...
  // fail open so the provider remains recoverable from the UI.
...
// Copilot authorization check: reuses the same token resolution logic
// (including ~/.config/github-copilot/apps.json and ghp_ PAT skipping) as
// getCopilotModels, so we never end up with "models can be fetched but the
// caller is told the account is unauthorized".
...
async function buildAvailableForProfile(
  ...
  for (const [providerKey, envMapping] of Object.entries(PROVIDER_ENV_MAP)) {
    const oauthAuthorized = isOAuthAuthorized(providerKey)
...
export async function getAvailable(ctx: any) {
  try {
    ...
    // Reuse the Copilot dynamic model list within this single request
    // (getCopilotModelsDetailed already has its own inflight + cache; we keep
    // a local ref here to further reduce branching).
    ...
    // Copilot is explicit opt-in: even if a token resolves, we only return
    // models when the user has enabled Copilot through ui's Add Provider
    // flow. Prevents a VS Code / gh CLI user's global credentials from being
    // surfaced as a hermes provider by accident.
    const appConfig = await readAppConfig()
    const copilotEnabled = appConfig.copilotEnabled === true
    const modelAliases = normalizeAliases(appConfig.modelAliases)
    const modelVisibility = normalizeModelVisibility(appConfig.modelVisibility)
    ...
    // Backwards compatibility for older users: the previous version auto-fell
    // back to Copilot discovery. After upgrade, those users' config.yaml may
    // still point model.default at a Copilot model. If Copilot is no longer
    // enabled, clear the returned default and let the front-end pick the first
    // available model from the remaining providers.
    if (!copilotEnabled && currentDefaultProvider.toLowerCase() === 'copilot') {
      currentDefault = ''
      currentDefaultProvider = ''
    }
    ...
    // Pull the live Copilot model list once for the allProviders view
    // (within this request, the cache is reused). Skip the fetch entirely when
    // Copilot is not enabled, to avoid a useless network round-trip.
...
/**
 * Upsert the model context configuration (UPSERT: update if exists, insert if not).
 * Accepts parameters via path or query string.
 */
export async function updateModelContext(ctx: any) {
  // Two supported parameter styles:
  // 1. Path params: /api/hermes/model-context/:provider/:model
  // 2. Query params: /api/hermes/model-context?provider=xxx&model=xxx
  let provider: string | undefined
  let model: string | undefined

  // Prefer path params when present
  ...
  // Final fallback: read from the request body
  if (!provider || !model) {
...
/**
 * Fetch the model context configuration.
 */
