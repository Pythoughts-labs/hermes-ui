...
function resolveActiveTtsProvider(profile: string, userId: number | null, settings?: ReturnType<typeof listTtsProviderSettings>) {
  ...
  const configured = (settings ?? listTtsProviderSettings(profile)).filter(setting => setting.provider !== 'local')
  return configured.length === 1 ? configured[0].provider : 'local'
}
...
export async function deleteProvider(ctx: Context) {
  ...
  try {
    ...
    const activeProvider = currentActiveProvider === storedProvider
      ? saveActiveTtsProvider(profile, 'local')
...
function summarizeProbeError(error: unknown): { summary: string; details: string } {
  ...
  if (/fetch failed|network|ENOTFOUND|ECONNREFUSED|ECONNRESET/i.test(message)) {
    return { summary: 'Could not reach the provider from the UI server. Check the Base URL and network access.', details: message }
  }
...
export async function synthesize(ctx: Context) {
  ...
  try {
    ...
    // ponytail: clear the audio Content-Type set on the success path so
    // the client doesn't try to parse a JSON error body as audio/wav.
