...
const UI_LOG_FILE = join(config.appHome, 'logs', 'server.log')
...
function parseLine(line: string): LogEntry {
  try {
    ...
    if (obj.level && obj.time) {
      const ts = new Date(obj.time).toLocaleString('en-US', { hour12: false }).replace(/\//g, '-')
      const levelMap: Record<number, string> = { 10: 'TRACE', 20: 'DEBUG', 30: 'INFO', 40: 'WARN', 50: 'ERROR', 60: 'FATAL' }
      // Pino log shape: { level, time, msg, name (logger name), hostname, pid, ... }
...
export async function read(ctx: any) {
  ...
  if (logName === 'ui') {
    try {
      if (!existsSync(UI_LOG_FILE)) { ctx.body = { entries: [] }; return }
      const content = await readFile(UI_LOG_FILE, 'utf-8')
