...
function isVisibleUiSessionSource(source?: string | null): boolean {
...
function isRequestedSessionSource(source: string | undefined, sessionSource?: string | null): boolean {
  ...
  return isVisibleUiSessionSource(sessionSource)
}
...
export async function listHermesSessions(ctx: any) {
  ...
  const allSessions = (await listSessionSummaries(source, effectiveLimit, profile))
    .map(session => ({
      ...
      ui_imported: importedIds.has(session.id),
    }))
  ...
  for (const session of localSessions) {
    ...
    historySessionsById.set(session.id, { ...session, ui_imported: true })
  }
  ctx.body = {
...
export async function getHermesSession(ctx: any) {
  ...
  // Prefer the UI local session store. Hermes state.db can lag behind or
  ...
  // Filter out UI-only session sources.
...
export async function exportSession(ctx: any) {
  ...
  // Replace anything that is not a safe filename character with '_'. Allow
  // letters, digits, underscore, hyphen, dot, and space. Non-ASCII characters
  // are also retained so titles in other scripts remain readable.
