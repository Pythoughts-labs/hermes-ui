 ...
 *
 * Useful when hermes-ui is bundled inside a packaged distribution
 * (e.g. a desktop app) where the user can't `npm install -g @pythoughts/hermes-ui@latest`
 ...
 *
 * Set HERMES_UI_DISABLE_UPDATE_CHECK=true (or 1, on, yes) to disable.
 */
function isUpdateCheckDisabled(): boolean {
  const raw = (process.env.HERMES_UI_DISABLE_UPDATE_CHECK || '').trim().toLowerCase()
...
export async function checkLatestVersion(): Promise<void> {
  ...
  if (!PACKAGE_INFO?.name) return
  try {
    const res = await fetch(`https://registry.npmjs.org/${PACKAGE_INFO.name}/latest`, {
      signal: AbortSignal.timeout(10000),
    })
    if (res.ok) {
      const data = await res.json() as { version?: string }
      const latest = (data.version || '').trim().replace(/^v/i, '')
      if (latest) {
        cachedLatestVersion = latest
        if (LOCAL_VERSION && isNewerVersion(latest, LOCAL_VERSION)) {
...
export async function healthCheck(ctx: any) {
  ...
  ctx.body = {
    ...
    gateway: 'running',
    ui_version: LOCAL_VERSION,
    ui_latest: isUpdateCheckDisabled() ? '' : cachedLatestVersion,
    ui_update_available: isUpdateCheckDisabled()
      ? false
