...
export interface GatewayAutoStartConfig {
  ...
  // controller. It is not persisted in the UI app config.
...
export interface AppConfig {
  // Whether GitHub Copilot has been explicitly added by the user in ui.
  ...
  // via "Add Provider". Mirrors how the user manages Codex/Nous: the ui
  // owns the provider list, system credentials are merely a fallback source.
  copilotEnabled?: boolean

  // UI-only model display aliases. Keys are provider -> canonical model ID -> display label.
  // These aliases never replace the canonical model ID sent back to Hermes.
  modelAliases?: Record<string, Record<string, string>>

  // UI-only manually entered model IDs. Keys are provider -> model IDs.
  ...
  // UI-only model picker visibility. This filters what the WUI exposes in
  ...
  // UI startup policy for automatically starting Hermes API gateways.
  // Defaults to legacy behavior: all local profiles are eligible. This is a
  // UI-level setting, not the active Hermes profile's config.yaml.
