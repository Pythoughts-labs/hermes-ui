...
const MCU_TTS_FAILED_PROMPT_TEXT = 'Text-to-speech synthesis failed. Configure a text-to-speech provider in Settings before retrying.'
const MCU_TTS_FAILED_PROMPT_PCM_URL =
  '/audio/tts-synthesize-failed.s16le.pcm'
...
class PlainWebSocketRelayClient {
  ...
  private async synthesizeMcuSpeech(text: string, profile: string): Promise<{ url: string }> {
    if (!this.options.userToken) {
      throw new Error('missing UI auth token')
    }
