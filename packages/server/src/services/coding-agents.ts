...
const LEGACY_HERMES_MCP_SERVER_NAMES = new Set(['hermes-ui', 'hermes-ui-mcp'])
const LEGACY_HERMES_MCP_COMMANDS = new Set([
  'hermes-lan-peer-mcp',
  'hermes-devices-mcp',
  'hermes-ui-mcp',
])
const HERMES_MCP_MANAGED_ENV_KEY = 'HERMES_UI_MANAGED_MCP'
const HERMES_PROMPT_BLOCK_BEGIN = '<!-- BEGIN HERMES UI PROMPT -->'
const HERMES_PROMPT_BLOCK_END = '<!-- END HERMES UI PROMPT -->'
...
function getScopedConfigRoot(id: CodingAgentId, scope: Required<CodingAgentConfigScope>): string {
  return join(getUiHome(), CODING_AGENT_HOME_DIR, 'model', scope.profile, scope.provider, id)
}

function getScopedWorkspaceRoot(scope: Required<CodingAgentConfigScope>): string {
  return join(getUiHome(), CODING_AGENT_HOME_DIR, 'workspace', scope.profile, scope.provider)
}
...
function hermesMcpCommandConfig(toolset: string): { command: string; args?: string[] } {
  if (isDesktopRuntime()) return { command: 'hermes-ui-mcp', args: [toolset] }
...
function hermesMcpServerConfig(profile: string, serverName: string, toolset: string): { command: string; args?: string[]; env: Record<string, string> } {
  ...
  return {
    ...
    env: {
      HERMES_UI_URL: `http://127.0.0.1:${process.env.PORT || '8648'}`,
      HERMES_UI_HOME: appHome,
      HERMES_UI_STATE_DIR: appHome,
