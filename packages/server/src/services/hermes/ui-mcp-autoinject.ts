...
const LEGACY_SERVER_NAME = 'hermes-ui'
const MANAGED_SERVERS = [
  { name: 'hermes-ui-api', toolset: 'api' },
  { name: 'hermes-ui-devices', toolset: 'devices' },
  { name: 'hermes-ui-use', toolset: 'use' },
] as const
...
const MANAGED_ENV_KEY = 'HERMES_UI_MANAGED_MCP'
...
function isDisabled(): boolean {
  return isEnabledEnv(process.env.HERMES_UI_DISABLE_MCP_AUTOINJECT)
}

function allowTransientAutoinject(): boolean {
  return isEnabledEnv(process.env.HERMES_UI_ALLOW_TRANSIENT_MCP_AUTOINJECT)
}
...
function candidateBundledMcpScripts(): string[] {
  return [
    process.env.HERMES_UI_MCP_BIN,
    join(process.cwd(), 'bin/hermes-ui-mcp.mjs'),
    join(__dirname, '../../bin/hermes-ui-mcp.mjs'),
    join(__dirname, '../../../../../bin/hermes-ui-mcp.mjs'),
...
function managedCommandConfig(toolset: string): Record<string, unknown> {
  if (isDesktopRuntime()) {
    return { command: 'hermes-ui-mcp', args: [toolset] }
  }
...
function managedConfig(profile: string, serverName: string, toolset: string): Record<string, unknown> {
  const env: Record<string, string> = {
    HERMES_UI_URL: `http://127.0.0.1:${config.port}`,
    HERMES_UI_HOME: config.appHome,
    HERMES_UI_STATE_DIR: config.appHome,
    HERMES_UI_PROFILE: profile,
...
function sameConfig(existing: Record<string, any>, desired: Record<string, unknown>): boolean {
  ...
  return existing.command === desired.command &&
    ...
    existing.env.HERMES_UI_URL === desiredEnv.HERMES_UI_URL &&
    existing.env.HERMES_UI_HOME === desiredEnv.HERMES_UI_HOME &&
    existing.env.HERMES_UI_STATE_DIR === desiredEnv.HERMES_UI_STATE_DIR &&
    existing.env.HERMES_UI_PROFILE === desiredEnv.HERMES_UI_PROFILE &&
    ...
    existing.env.HERMES_UI_TOKEN === undefined &&
      ...
      if (LEGACY_SERVER_NAMES.has(name) && !isManagedServer(server)) {
        return {
          ...
          result: {
            profile,
            status: 'skipped',
            reason: `existing ${name} MCP server is not managed by Hermes UI`,
      ...
      if (!isManagedServer(existing)) {
        return {
          ...
          result: {
            profile,
            status: 'skipped',
            reason: `existing ${server.name} MCP server is not managed by Hermes UI`,
          } satisfies BundledMcpInjectionTargetResult,
        }
      }
...
export async function injectBundledMcpServer(): Promise<BundledMcpInjectionResult> {
  ...
  if (isDisabled()) {
    logger.info('[mcp-autoinject] disabled by HERMES_UI_DISABLE_MCP_AUTOINJECT')
    return result
  }

  if (shouldSkipTransientAutoinject()) {
    logger.info({ appHome: config.appHome }, '[mcp-autoinject] skipped for transient UI home')
    return result
  }
