...
def _candidate_agent_roots(raw: str | None = None) -> list[Path]:
    ...
    if hermes_bin:
        ...
        # uv tool install layout: after resolving the shim symlink,
        # bin_path sits at <prefix>/share/uv/tools/hermes-agent/bin/hermes
        # and run_agent.py is at .../lib/python*/site-packages/run_agent.py.
        uv_lib = bin_path.parent.parent / "lib"
        if uv_lib.is_dir():
            for version_dir in uv_lib.glob("python*"):
...
def _apply_profile_dotenv(profile: str | None) -> dict[str, str | None]:
    ...
    This mirrors UI gateway env isolation:
...
def _install_execute_code_approval_memory_patch() -> None:
    ...
    gateway/ask mode.  UI keeps HERMES_EXEC_ASK enabled so dangerous
    ...
    try:
        ...
        if not callable(original) or getattr(original, "_hermes_ui_memory_patch", False):
            return
        ...
        setattr(patched_check_execute_code_guard, "_hermes_ui_memory_patch", True)
        setattr(patched_check_execute_code_guard, "_hermes_ui_original", original)
