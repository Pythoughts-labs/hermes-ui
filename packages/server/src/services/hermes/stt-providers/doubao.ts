...
const DEFAULT_USER_ID = 'hermes-ui'
...
function unsupportedAudioError(input: SttTranscribeInput): SttProviderConfigError {
  return new SttProviderConfigError(
    `Doubao STT cannot upload ${describeInputAudio(input)} directly. Use WAV/MP3/OGG/OPUS audio, or install ffmpeg on the UI server so browser WebM/Opus recordings can be transcoded to WAV.`,
  )
}
