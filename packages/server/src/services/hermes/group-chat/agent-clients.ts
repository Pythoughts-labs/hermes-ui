    ...
    async replyToMention(
        ...
        try {
            ...
            const routedPrefix = isAllAgentsMentioned(msg.content)
                ? `Group chat system: this message @-mentions every agent via @all, and you are one of them — please reply directly.`
                : `Group chat system: this message mentions you (${this.name}) — please reply directly. Even if it also mentions other members, do not refuse to reply or produce an empty reply.`
            ...
            const runPrompt = 'When calling Hermes UI endpoints from tools or skills, include the current Hermes profile as the X-Hermes-Profile header if the endpoint supports profile-scoped behavior.'
