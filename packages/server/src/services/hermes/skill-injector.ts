...
const MANIFEST_FILENAME = '.ui-managed-skills.json'
const MANIFEST_OWNER = 'hermes-ui'
...
export class HermesSkillInjector {
  ...
  static resolveSourceDir(env: NodeJS.ProcessEnv = process.env, baseDir = __dirname): string {
    const override = env.HERMES_UI_SKILLS_DIR?.trim()
  ...
  private async injectIntoTarget(targetDir: string, entries: import('fs').Dirent[]): Promise<SkillInjectionTargetResult> {
    ...
    for (const entry of entries) {
      ...
      logger.warn({
        ...
        skill: entry.name,
        targetSkillDir,
      }, '[skill-injector] skipped bundled skill because target was not an unchanged UI-managed copy')
    }
