...
const HERMES_UI_NAMESPACE = 'mcp__hermes_ui'

const HERMES_UI_MCP_TOOLS = [
  {
...
const HERMES_UI_MCP_TOOL_NAMES = new Set(HERMES_UI_MCP_TOOLS.map(tool => tool.name))
...
function expandedResponseTools(tools: unknown): any[] {
  ...
  for (const tool of tools) {
    ...
    if (tool?.type === 'namespace' && normalizedNamespaceName(tool?.name) === HERMES_UI_NAMESPACE) {
      for (const mcpTool of HERMES_UI_MCP_TOOLS) {
        addFunctionTool({
          type: 'function',
          name: mcpTool.name,
          description: `${mcpTool.description} MCP namespace: ${HERMES_UI_NAMESPACE}.`,
          parameters: mcpTool.inputSchema,
          namespace: HERMES_UI_NAMESPACE,
        })
      }
      continue
    }
...
export function responseToolNamespaceForName(name: unknown): string | undefined {
  return HERMES_UI_MCP_TOOL_NAMES.has(String(name || '')) ? HERMES_UI_NAMESPACE : undefined
}
