...
import { getHermesUiVersion } from './system-info'

const ACTIVE_VERSION_FILE = 'active-version.json'
const DEFAULT_REMOTE_MANIFEST_URL =
  'https://raw.githubusercontent.com/Pythoughts-labs/hermes-ui/main/packages/website/public/versions.json'
const DEFAULT_GITHUB_REPO = 'Pythoughts-labs/hermes-ui'
...
function downloadAssetUrl(assetName: string, tag: string): string {
  const repo = process.env.HERMES_UI_DOWNLOAD_GITHUB_REPO?.trim() || DEFAULT_GITHUB_REPO
...
export function listInstalledUiVersions(active = readActiveVersionManifest()): InstalledUiVersion[] {
  const root = join(config.appHome, 'ui')
  if (!existsSync(root)) return []

  const activeDir = active?.uiDirectory ? resolve(active.uiDirectory) : ''
  const installed: InstalledUiVersion[] = []
...
async function fetchRemoteVersions(): Promise<{ manifest: RemoteVersionManifest | null; error: string }> {
  const url = process.env.HERMES_UI_VERSION_MANIFEST_URL?.trim() || DEFAULT_REMOTE_MANIFEST_URL
  try {
...
export async function getRuntimeVersionStatus(): Promise<RuntimeVersionStatus> {
  ...
  const uiVersion = getHermesUiVersion()

  return {
    active,
    platform: runtimePlatformKey(),
    activeVersionPath: activeVersionPath(),
    remoteManifestUrl: process.env.HERMES_UI_VERSION_MANIFEST_URL?.trim() || DEFAULT_REMOTE_MANIFEST_URL,
    remoteError: error,
    hermes: {
    ...
    ui: {
      ...
      activeVersion: active?.uiVersion || uiVersion,
      activeDirectory: active?.uiDirectory || '',
      installed: listInstalledUiVersions(active),
      remoteVersions: normalizeStringList(manifest?.ui),
    },
  }
}
...
export async function downloadRuntimeVersion(version: string, onProgress?: DownloadProgressHandler): Promise<InstalledRuntimeVersion> {
  ...
  const manifestUrl = downloadAssetUrl(manifestName, releaseTag)
  ...
  const assetUrl = downloadAssetUrl(assetName, releaseTag)
...
export async function downloadUiVersion(version: string, onProgress?: DownloadProgressHandler): Promise<InstalledUiVersion> {
  const cleanVersion = version.trim().replace(/^v/, '')
  if (!cleanVersion) throw new Error('UI version is required')

  const releaseTag = `v${cleanVersion}`
  const assetName = `hermes-ui-${cleanVersion}.tar.gz`
  const manifestName = `hermes-ui-${cleanVersion}.json`
  const manifestUrl = downloadAssetUrl(manifestName, releaseTag)
  onProgress?.({ stage: 'resolve', message: 'runtimeVersions.jobStage.resolveUi' })
  ...
  const tempRoot = join(desktopRuntimeRoot(), `.ui-download-${process.pid}-${Date.now()}`)
  const targetRoot = join(config.appHome, 'ui', cleanVersion)
  const assetUrl = downloadAssetUrl(assetName, releaseTag)
  ...
  try {
    await downloadFile(assetUrl, archive, onProgress)
    onProgress?.({ stage: 'verify', message: 'runtimeVersions.jobStage.verifyUi', percent: 100 })
    if (manifest.asset?.sha256) {
      const actual = await sha256File(archive)
      if (actual !== manifest.asset.sha256) throw new Error(`UI checksum mismatch for ${assetName}`)
    }
    onProgress?.({ stage: 'extract', message: 'runtimeVersions.jobStage.extractUi' })
    await extractTarGzip(archive, tempRoot)
    const extractedRoot = join(tempRoot, 'ui')
    for (const required of ['package.json', 'bin/hermes-ui.mjs', 'dist/server/index.js']) {
      if (!existsSync(join(extractedRoot, required))) throw new Error(`UI archive is missing required file: ${required}`)
    }
    onProgress?.({ stage: 'install', message: 'runtimeVersions.jobStage.installUi' })
    rmSync(targetRoot, { recursive: true, force: true })
    mkdirSync(dirname(targetRoot), { recursive: true })
...
export function activateInstalledRuntimeVersion(version: string): ActiveVersionManifest {
  ...
  const next: ActiveVersionManifest = {
    ...
    uiVersion: active?.uiVersion || getHermesUiVersion(),
    runtimeDirectory: target.directory,
    uiDirectory: active?.uiDirectory || '',
...
export function activateDownloadedUiVersion(version: string): ActiveVersionManifest {
  const cleanVersion = version.trim().replace(/^v/, '')
  if (!cleanVersion) throw new Error('UI version is required')
  const directory = join(config.appHome, 'ui', cleanVersion)
  if (!existsSync(join(directory, 'package.json'))) throw new Error(`Downloaded UI version not found: ${cleanVersion}`)
  const active = readActiveVersionManifest()
  const next: ActiveVersionManifest = {
    schema: 1,
    hermesRuntimeVersion: active?.hermesRuntimeVersion || '',
    uiVersion: cleanVersion,
...
export function deleteDownloadedUiVersion(version: string): InstalledUiVersion {
  const cleanVersion = version.trim().replace(/^v/, '')
  if (!cleanVersion) throw new Error('UI version is required')

  const active = readActiveVersionManifest()
  const installed = listInstalledUiVersions(active)
  const target = installed.find(item => item.version === cleanVersion)
  if (!target) throw new Error(`Downloaded UI version not found: ${cleanVersion}`)
  if (target.active) throw new Error('Active UI version cannot be deleted')

  const uiRoot = resolve(join(config.appHome, 'ui'))
  const targetDir = resolve(target.directory)
  const rel = relative(uiRoot, targetDir)
  if (!rel || rel.startsWith('..') || rel === '..') {
    throw new Error('Only downloaded UI versions can be deleted')
  }
...
function createDownloadJob(
  kind: VersionDownloadKind,
  version: string,
  runner: (cleanVersion: string, onProgress: DownloadProgressHandler) => Promise<InstalledRuntimeVersion | InstalledUiVersion>,
): VersionDownloadJob {
  const cleanVersion = version.trim().replace(/^v/, '')
  if (!cleanVersion) throw new Error(`${kind === 'runtime' ? 'Runtime' : 'UI'} version is required`)
  ...
  queueMicrotask(() => {
    ...
    updateProgress({
      stage: 'resolve',
      message: kind === 'runtime' ? 'runtimeVersions.jobStage.resolveRuntime' : 'runtimeVersions.jobStage.resolveUi',
    })
...
export function startRuntimeVersionDownload(version: string): VersionDownloadJob {
  return createDownloadJob('runtime', version, (cleanVersion, onProgress) => downloadRuntimeVersion(cleanVersion, onProgress))
}

export function startUiVersionDownload(version: string): VersionDownloadJob {
  return createDownloadJob('ui', version, (cleanVersion, onProgress) => downloadUiVersion(cleanVersion, onProgress))
}
