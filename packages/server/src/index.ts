...
import { config, shouldCreateUiDataDir } from './config'
...
import { injectBundledMcpServer } from './services/hermes/ui-mcp-autoinject'
...
/**
 * Safely read network interface information (Termux/proot compatible).
 * Inside proot environments, os.networkInterfaces() throws a permission error
 * (errno 13); swallow that and return an empty object so callers can fall back.
 */
...
function gatewayAutostartDisabled(): boolean {
  return envFlagEnabled('HERMES_UI_DISABLE_GATEWAY_AUTOSTART')
}

function skillInjectionDisabled(): boolean {
  return envFlagEnabled('HERMES_UI_DISABLE_SKILL_INJECTION')
}

async function startRuntimeServicesBeforeListen(): Promise<void> {
  if (gatewayAutostartDisabled()) {
    console.log('[bootstrap] profile gateway check disabled by HERMES_UI_DISABLE_GATEWAY_AUTOSTART')
  } else {
...
export async function bootstrap() {
  console.log(`hermes-ui v${APP_VERSION} starting...`)
  await mkdir(config.uploadDir, { recursive: true })
  if (shouldCreateUiDataDir()) {
    await mkdir(config.dataDir, { recursive: true })
  }

  await initLoginLimiter()
  if (skillInjectionDisabled()) {
    console.log('[bootstrap] bundled skill injection disabled by HERMES_UI_DISABLE_SKILL_INJECTION')
  } else {
    try {
  ...
  const app = new Koa()
  // Initialize all ui SQLite tables
...
bootstrap().catch((error) => {
  console.error('FATAL: Failed to start Hermes UI')
  console.error(error)
