export default {
  ...
  login: {
    ...
    defaultCredentialsHint: '기본 로그인 이름: admin, 기본 비밀번호: admin',
  ...
  sidebar: {
    ...
    connected: '연결됨',
    disconnected: '연결 끊김',
    updateTip: '터미널에서 "hermes-ui update"를 실행하여 업데이트하세요',
  ...
  runtimeVersions: {
    ...
    jobStage: {
      ...
      resolveUi: 'UI 패키지 확인 중',
      verifyUi: 'UI 압축 파일 검증 중',
      extractUi: 'UI 압축 파일 해제 중',
      installUi: 'UI 설치 중',
      completed: '완료',
      failed: '실패',
    },
  ...
  chat: {
    ...
    sessionScopeHint: '채팅에는 UI/API Server 세션만 표시됩니다. CLI, Telegram, Discord, Cron 등 채널 세션은 기록에서 읽기 전용으로 볼 수 있습니다.',
    openHistory: '기록 열기',
    ...
    importToUi: 'UI로 가져오기',
    importSessionSuccess: '세션을 UI로 가져왔습니다',
    importSessionAlreadyExists: '세션이 이미 UI에 있습니다',
    ...
    searchScope: '검색 범위: UI 로컬 세션 DB만 포함하며 읽기 전용 Hermes 기록 세션은 포함하지 않습니다.',
  ...
  models: {
    ...
    visibilityHint: 'UI의 모델 선택기와 모델 페이지 표시에만 영향을 줍니다. Hermes CLI의 provider/model 설정은 변경되지 않으며 실제 호출은 원본 모델 ID를 사용합니다.',
    ...
    aliasManage: '표시 이름',
    aliasHint: 'UI 표시 이름만 변경합니다. Hermes에는 여전히 원본 모델 ID가 전달됩니다.',
    aliasEdit: '이름 변경',
  ...
  profiles: {
    ...
    avatar: {
      ...
      presetsHint: "갤러리에서 사전 설정된 아바타 선택",
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 자동 시작',
      description: 'UI 시작 시 자동으로 실행할 로컬 Hermes profile gateway를 제어합니다. 이 값은 profile config.yaml이 아니라 UI 설정에 저장됩니다.',
      enabled: 'Gateway 자동 시작',
      enabledHint: '끄면 시작 시 어떤 profile gateway도 자동으로 실행하지 않습니다.',
      management: '통합 gateway',
      managementHint: 'Hermes Agent 0.17.0 이상이 필요합니다. 켜면 UI가 multiplex_profiles를 기록해 default gateway가 모든 profiles를 처리합니다. 다른 profile은 Feishu, Weixin, QQBot 같은 메시징 진입점을 별도로 설정할 수 없습니다. 끄면 해당 설정을 제거합니다.',
      managementAuto: '자동',
    ...
    voice: {
      ...
      presetLocalTtsDescription: 'Microsoft Edge의 무료 고품질 클라우드 음성입니다.',
      ...
      sttFailureHintNetworkCustom: '서버가 사용자 지정 STT 엔드포인트에 연결할 수 없습니다. {endpoint}가 UI 서버에서 접근 가능하고 OpenAI 호환인지 확인하세요.',

      sttFailureHintNetwork: 'UI 서버가 STT 제공자에 연결할 수 없습니다. 네트워크/프록시/방화벽을 확인하고 다시 시도하세요.',
  ...
  githubPreview: {
    title: "버전 미리보기",
    description: "선택한 GitHub tag 를 UI 미리보기 작업 디렉터리에 클론하고, 의존성을 설치한 뒤 개발 포트로 실행합니다.",
    refresh: "새로고침",
    selectTag: "tag 선택",
    prepare: "코드 준비",
    install: "의존성 설치",
    start: "미리보기 시작",
    stop: "중지",
    note: "미리보기 코드는 UI 데이터 홈 아래에 저장됩니다. 프로덕션은 8648을 유지하고, 미리보기 개발 환경은 프론트엔드 8651, 백엔드 8650에서 실행됩니다.",
    path: "미리보기 경로",
    uiHome: "미리보기 데이터 홈",
    currentTag: "현재 Tag",
    repoReady: "저장소 준비됨",
  ...
  changelog: {
    ...
    new_0_6_18_2: 'Hermes UI MCP가 api, devices, use toolset으로 나뉘고 session, model, chat-run, ops 작업이 추가되었습니다',
    ...
    new_0_6_16_7: '큰 정적 assets는 동적 Brotli 압축을 피해서 부하 상황에서 packaged UI 전달이 더 안정적입니다',
    new_0_6_16_8: 'connector 형태의 연동을 위해 outbound relay와 MCU login flow를 추가했습니다',
    new_0_6_16_9: 'UI CLI에 scripts와 headless 실행용 --no-open flag를 추가했습니다',
    ...
    new_0_6_13_5: '데스크톱 업데이트 안정성을 개선했습니다. Windows 업그레이드는 남아 있는 Hermes UI 프로세스를 닫고, Cloudflare를 먼저 확인하며, GitHub 업데이트 피드를 fallback으로 유지합니다',
    new_0_6_13_6: '릴리스 자동화를 조정해 UI와 Docker release는 GitHub Latest로 표시하지 않고, 전체 데스크톱 release만 수동 빌드 성공 후 Latest로 승격합니다',
  },
