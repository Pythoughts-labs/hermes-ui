export default {
  ...
  login: {
    ...
    defaultCredentialsHint: '既定のユーザー名：admin、既定のパスワード：admin',
  ...
  sidebar: {
    ...
    connected: '接続済み',
    disconnected: '未接続',
    updateTip: 'ターミナルで "hermes-ui update" を実行して更新してください',
  ...
  runtimeVersions: {
    ...
    jobStage: {
      ...
      resolveUi: 'UI パッケージを解決中',
      verifyUi: 'UI アーカイブを検証中',
      extractUi: 'UI アーカイブを展開中',
      installUi: 'UI をインストール中',
      completed: '完了',
      failed: '失敗',
    },
  },
  ...
  chat: {
    ...
    sessionScopeHint: 'チャットには UI/API Server セッションのみ表示されます。CLI、Telegram、Discord、Cron などのチャンネルセッションは履歴で読み取り専用として表示されます。',
    openHistory: '履歴を開く',
    ...
    importToUi: 'UI にインポート',
    importSessionSuccess: 'セッションを UI にインポートしました',
    importSessionAlreadyExists: 'セッションは既に UI に存在します',
    ...
    searchScope: '検索範囲: UI のローカルセッション DB のみ。読み取り専用の Hermes 履歴セッションは含まれません。',
  ...
  models: {
    ...
    visibilityHint: 'UI のモデル選択とモデルページの表示だけに影響します。Hermes CLI の provider/model 設定は変更されず、実際の呼び出しは元のモデル ID を使います。',
    ...
    aliasManage: '表示名',
    aliasHint: 'UI の表示名だけを変更します。Hermes には元のモデル ID が送信されます。',
    aliasEdit: '名前変更',
  ...
  profiles: {
    ...
    avatar: {
      ...
      presetsHint: "ギャラリーからプリセットアバターを選択",
  ...
  settings: {
    ...
    gatewayAutoStart: {
      title: 'Gateway 自動起動',
      description: 'UI 起動時に自動起動するローカル Hermes profile gateway を制御します。この設定は UI 設定に保存され、profile config.yaml には保存されません。',
      enabled: 'Gateway を自動起動',
      enabledHint: '無効にすると、起動時に profile gateway を自動起動しません。',
      management: '統一 gateway',
      managementHint: 'Hermes Agent 0.17.0 以降が必要です。有効にすると UI が multiplex_profiles を書き込み、default gateway がすべての profiles を処理します。他の profile では Feishu、Weixin、QQBot などのメッセージング入口を個別設定できません。無効にするとその設定を削除します。',
      managementAuto: '自動',
    ...
    voice: {
      ...
      presetLocalTtsDescription: 'Microsoft Edge の無料高品質クラウド音声です。',
      ...
      sttFailureHintNetworkCustom: 'サーバーがカスタム STT エンドポイントに到達できませんでした。{endpoint} が UI サーバーから到達可能で OpenAI 互換か確認してください。',

      sttFailureHintNetwork: 'UI サーバーが STT プロバイダーに到達できませんでした。ネットワーク/プロキシ/ファイアウォールを確認して再試行してください。',
  ...
  githubPreview: {
    title: "バージョンプレビュー",
    description: "選択した GitHub tag を UI のプレビュー作業ディレクトリへクローンし、依存関係をインストールして開発ポートで起動します。",
    refresh: "更新",
    selectTag: "tag を選択",
    prepare: "コードを準備",
    install: "依存関係をインストール",
    start: "プレビューを開始",
    stop: "停止",
    note: "プレビューコードは UI データホーム配下に保存されます。本番は 8648 のまま、プレビュー開発環境はフロントエンド 8651、バックエンド 8650 で実行されます。",
    path: "プレビューパス",
    uiHome: "プレビューデータホーム",
    currentTag: "現在の Tag",
    repoReady: "リポジトリ準備済み",
  ...
  changelog: {
    ...
    new_0_6_18_2: 'Hermes UI MCP を api、devices、use の toolset に分割し、session、model、chat-run、ops 操作を追加しました',
    ...
    new_0_6_16_7: '大きな静的 assets は動的 Brotli 圧縮を避け、負荷下で packaged UI の配信が安定します',
    new_0_6_16_8: 'connector 系連携向けに outbound relay と MCU login flow を追加しました',
    new_0_6_16_9: 'UI CLI に scripts や headless 起動向けの --no-open フラグを追加しました',
    ...
    new_0_6_13_5: 'デスクトップ更新の信頼性を改善しました。Windows アップグレード時に残留 Hermes UI プロセスを閉じ、Cloudflare を先に確認し、GitHub をフォールバック更新元として保持します',
    new_0_6_13_6: 'リリース自動化を調整し、UI と Docker の release は GitHub Latest にせず、完全なデスクトップ release だけを手動ビルド成功後に Latest へ昇格します',
  },
