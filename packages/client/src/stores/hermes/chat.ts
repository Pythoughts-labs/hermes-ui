...
export interface Message {
  ...
  toolDuration?: number  // Tool execution duration in seconds
  isStreaming?: boolean
  attachments?: Attachment[]
  // Thinking/reasoning text. Two sources:
  //   1) Historical messages: from the HermesMessage.reasoning field
  //   2) Streaming: accumulated from reasoning.delta / thinking.delta / reasoning.available events
  // Does not include the <think> wrapper tags; the content itself can be multiple plain-text segments.
  reasoning?: string
  queued?: boolean
...
// Get the current profile name to scope the cache.
// Reads from profiles store's activeProfileName (synchronous localStorage),
// to avoid null at chat store init time due to async loading.
...
function recoverStorageQuota() {
  try {
    // Clean up all session-related legacy cache (fully deprecated)
    const prefixes = [
...
export const useChatStore = defineStore('chat', () => {
          ...
          switch (evt.event) {
            ...
            case 'reasoning.available': {
              ...
              if (last?.role === 'assistant' && last.isStreaming) {
                // Only mark as ended when a reasoning.delta event previously started the timer;
                // otherwise (upstream never forwarded delta, only this single available) do not show duration.
            ...
            case 'message.delta': {
              ...
              if (last?.role === 'assistant' && last.isStreaming) {
                ...
                // If reasoning was accumulated earlier, content arrival means the reasoning ended.
            ...
            case 'run.completed': {
              ...
              if (autoPlaySpeechEnabled.value && runProducedAssistantContent) {
                ...
                if (lastAssistant?.content) {
                  // Delay a short moment before playing so UI updates complete
                  setTimeout(() => {
  ...
  /** Mark startedAt the first time a message's reasoning text is seen. */
  ...
  /** Mark endedAt when content first arrives (treated as reasoning end) or reasoning.available is explicitly received. */
  ...
  function clearThinkingObservationFor(_sessionId: string) {
    // The messageId-to-sessionId association is not held separately; the design is to
    // clear everything when switching sessions. This matches the spec: observations
    // are transient state within the current session.
    thinkingObservation.clear()
  }

  // Play message voice
  function playMessageSpeech(messageId: string, content: string) {
    // Dispatch a custom event so the MessageItem component handles playback
