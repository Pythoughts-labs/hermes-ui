...
export const useAppStore = defineStore('app', () => {
  ...
  // uiVersion = version baked into this loaded bundle; serverVersion = version
  // reported by the running server. They differ while a release is rolling out.
  const uiVersion = ref(UI_VERSION)
  const serverVersion = ref(UI_VERSION)
  ...
  async function doUpdate(): Promise<boolean> {
    ...
    try {
      ...
      console.error('Failed to update Hermes UI:', err)
      return false
    } finally {
  ...
  async function checkConnection() {
    try {
      ...
      if (res.ui_version) serverVersion.value = res.ui_version
      // Only prompt a reload when the server is strictly newer than this loaded
      // bundle — then a reload actually fetches the matching client. If the bundle
      // is newer (e.g. mid-release the client deployed first), reloading wouldn't help.
      clientOutdated.value = !!res.ui_version
        && res.ui_version.localeCompare(UI_VERSION, undefined, { numeric: true }) > 0
      if (res.ui_latest) latestVersion.value = res.ui_latest
      updateAvailable.value = !!res.ui_update_available
