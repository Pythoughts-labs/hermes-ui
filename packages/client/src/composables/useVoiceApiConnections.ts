...
function isStoredTtsProvider(provider: VoiceApiProvider): provider is StoredTtsProvider {
  return provider === 'local' || provider === 'openai' || provider === 'custom' || provider === 'mimo' || provider === 'doubao'
}
...
export function useVoiceApiConnections() {
  ...
  const activeTtsId = computed(() => `tts-${vs.provider.value === 'webspeech' ? 'local' : vs.provider.value}`)
  ...
  function applyTtsConnectionToLegacyState(connection: VoiceApiConnection) {
    ...
    if (connection.provider === 'local') {
      vs.setLocalVoice(stringSetting(settings, 'voice') || connection.voice || vs.localVoice.value)
      const speed = Number(settings.speed)
      if (Number.isFinite(speed)) vs.setLocalSpeed(speed)
      return
    }
  ...
  async function refresh() {
    ...
    try {
      ...
      const localStatus = await fetchLocalTtsStatus().catch(() => null)
      ...
      const newConnections: VoiceApiConnection[] = [
        {
          ...
          kind: 'tts',
          provider: 'local',
          label: t('settings.voice.presetLocalTtsLabel'),
          isBuiltin: true,
          isLocal: true,
          hasSecret: false,
          active: activeTtsId.value === 'tts-local',
          modelState: localStatus?.model?.status,
          modelProgress: localStatus?.model?.status === 'downloading'
            ? { receivedBytes: localStatus.model.receivedBytes, totalBytes: localStatus.model.totalBytes }
            : undefined,
          installMessage: localStatus?.model?.status === 'installing'
            ? localStatus.model.message
            : undefined,
          settings: {
            voice: vs.localVoice.value,
            speed: vs.localSpeed.value,
          },
          voice: vs.localVoice.value,
        },
        {
  ...
  async function deleteSecret(kind: VoiceApiKind, provider: VoiceApiProvider) {
    if (kind === 'tts') {
      if (!isStoredTtsProvider(provider) || provider === 'local') return
