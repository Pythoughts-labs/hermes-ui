...
export interface SpeechOptions {
  lang?: string      // Language such as 'en-US', 'en-GB'
  voiceName?: string // Optional WebSpeech voice name
}
...
export interface OpenaiTtsOptions {
  ...
  model?: string
  voice?: string
  rate?: string   // Provider-specific rate (Edge format, e.g. "+20%")
  pitch?: string  // Provider-specific pitch (Edge format, e.g. "-8Hz")
  speed?: number  // Numeric speed multiplier (local TTS)
  stylePrompt?: string
  provider?: 'local' | 'openai' | 'custom' | 'doubao'
}
...
export interface SpeechState {
  ...
  progress: number  // Current progress (character count)
  engine: 'none' | 'tts' | 'browser'  // Currently active engine
}
...
/**
 * Voice playback composable.
 * Prefers backend TTS (Edge -> Google), and falls back to browser speechSynthesis on failure.
 */
export function useSpeech() {
  ...
  // Custom TTS (OpenAI / Custom / Edge) playback state
  ...
  /**
   * Extract plain-text content from the input text, filtering out code blocks,
   * thinking tags, and similar.
   */
  function extractReadableText(content: string): string {
    ...
    text = text.replace(/[^\p{L}\p{N}\s.!?;,'"(){}\[\]<>«»\u3010\u3011\u4e00-\u9fff\u3400-\u4dbf]/gu, '')
  ...
  function speakViaBrowser(messageId: string, text: string, options: SpeechOptions, token?: number) {
    ...
    // Use the specified voice if provided, otherwise fall back to the default
  ...
  return {
    ...
    // Browser WebSpeech (direct call to avoid Rolldown tree-shaking)
    speakViaBrowser,
  }
}
