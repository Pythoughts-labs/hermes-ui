...
<template>
  <div class="page-sidebar-nav">
    <div class="page-sidebar-tabs" role="tablist" aria-label="Chat actions">
      <button
        class="page-sidebar-tab--primary"
        type="button"
        @click="emit('primary')"
      >
        <svg
...
// ponytail: primary action pill — editorial new-task pill shape, design tokens.
.page-sidebar-tab--primary {
  display: flex;
  align-items: center;
...
// ponytail: light-mode primary is solid black/white by default. Hover lifts
// the border to a slightly lighter tone so the affordance is still visible.
// Dark mode keeps the original subtle tint (ask if you want it inverted).
:root[data-theme='light'] .page-sidebar-tab--primary {
  background: #000000;
  border-color: #000000;
  color: #ffffff;
}

:root[data-theme='light'] .page-sidebar-tab--primary:hover {
  background: #000000;
  border-color: #4b5563;
}

// ponytail: dark mode mirrors the Send button — white surface, dark text.
// Sizing matches Naive UI's small primary (h=36, radius=8, fontSize=14).
:root[data-theme='dark'] .page-sidebar-tab--primary {
  height: 36px;
  padding: 0 14px;
  border-radius: 8px;
  background: #ffffff;
...
:root[data-theme='dark'] .page-sidebar-tab--primary:hover {
  background: #eaeaea;
  border-color: #eaeaea;
  color: #121212;
}
