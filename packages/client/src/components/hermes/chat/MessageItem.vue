...
// Local TTS no longer needs edge-specific helpers — voice + speed are passed directly.
...
// Prefer the thinking text from the reasoning field/event; otherwise fall back to the
// <think> tags parsed from the content. If both coexist, concatenate them for display
// (rare, but keeps information from being lost).
...
// Streaming thinking state: an unclosed <think> tag is still present, or reasoning
// has content but the main body has not started.
...
const canPlaySpeech = computed(() => {
  // Only assistant messages can be played
  if (props.message.role !== 'assistant') return false
  if (!copyableContent.value) return false
  // OpenAI / Custom / Edge / MiMo / Doubao do not depend on the browser Web Speech API
  if (voiceSettings.provider.value === 'openai' || voiceSettings.provider.value === 'custom' || voiceSettings.provider.value === 'local' || voiceSettings.provider.value === 'mimo' || voiceSettings.provider.value === 'doubao') return true
  return speech.isSupported
})

const isPlayingThisMessage = computed(() => {
  // OpenAI / Custom / Local / MiMo / Doubao mode
  if (voiceSettings.provider.value === 'openai' || voiceSettings.provider.value === 'custom' || voiceSettings.provider.value === 'local' || voiceSettings.provider.value === 'mimo' || voiceSettings.provider.value === 'doubao') {
...
function handleSpeechToggle() {
  ...
  if (voiceSettings.provider.value === 'openai') {
    ...
    if (!apiUrl) {
      console.warn('[MessageItem] OpenAI TTS URL is empty')
      return
    }
  ...
  // Custom endpoint mode (OpenAI-compatible, e.g. GPT-SoVITS)
  if (voiceSettings.provider.value === 'custom') {
    const apiUrl = voiceSettings.customUrl.value
    if (!apiUrl) {
      console.warn('[MessageItem] Custom TTS URL is empty')
      return
    }
    speech.openaiToggle(props.message.id, content, {
      provider: 'custom',
  ...
  // Local TTS mode (Kokoro-82M, on-device)
  if (voiceSettings.provider.value === 'local') {
    speech.openaiToggle(props.message.id, content, {
      provider: 'local',
      baseUrl: '/api/tts/proxy',
      voice: voiceSettings.localVoice.value,
      speed: voiceSettings.localSpeed.value,
    })
    return
  }
...
onMounted(() => {
  autoPlayHandler = (e: Event) => {
    ...
    if (customEvent.detail.messageId === props.message.id && canPlaySpeech.value) {
      ...
      } else if (voiceSettings.provider.value === 'local') {
...
// Stop playback and clean up event listeners when the component unmounts
onBeforeUnmount(() => {
  if (autoPlayHandler) {
