...
// Edge-specific helpers no longer used — local TTS takes voice + speed directly.
...
// Find the record for the current message sender in members
...
// Parse the member's avatar JSON
...
// Avatar to display for the current message (profile / member / fallback)
...
const canPlaySpeech = computed(() => {
    ...
    if (voiceSettings.provider.value === 'openai' || voiceSettings.provider.value === 'custom' || voiceSettings.provider.value === 'local' || voiceSettings.provider.value === 'mimo') return true
    return speech.isSupported
})
const isPlayingThisMessage = computed(() => {
    if (voiceSettings.provider.value === 'openai' || voiceSettings.provider.value === 'custom' || voiceSettings.provider.value === 'local' || voiceSettings.provider.value === 'mimo') {
