...
watch(selectedPreset, (val) => {
  ...
  if (val) {
    ...
    if (val === COPILOT_KEY) {
      // Check if a token can already be resolved: yes -> simple confirmation dialog;
// no -> in-app device flow
...
async function handleSave() {
  ...
  // Codex: show the authorization code dialog
  if (isCodex.value) {
    showCodexLogin.value = true
    return
  }

  // Nous: show the OAuth device code dialog
  if (isNous.value) {
    showNousLogin.value = true
    return
  }

  // Copilot: token-aware add flow (token present -> confirmation dialog; otherwise device flow)
  if (isCopilot.value) {
    void triggerCopilotAdd()
    return
  }
...
async function triggerCopilotAdd() {
  ...
  try {
    ...
    if (status.has_token) {
      // Token already resolvable: show confirmation dialog, user clicks [Add] -> enable + saved
...
function handleCopilotClose() {
  showCopilotLogin.value = false
  // When the user cancels the Copilot onboarding, clear the selection to avoid
// being stuck in a no-api-key state
  selectedPreset.value = null
}
