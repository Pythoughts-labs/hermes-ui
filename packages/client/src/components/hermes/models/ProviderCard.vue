...
async function handleDelete() {
  let copilotMsg = ''
  if (isCopilot.value) {
    // Look up the source up front so the user knows whether removing it will affect
// the sign-in state of VS Code, gh CLI, or other tools
    try {
  ...
  dialog.warning({
    ...
    onPositiveClick: async () => {
      ...
      try {
        if (isCopilot.value) {
          // Copilot follows an explicit opt-in model: disable sets enabled to false,
          // and the token is only cleared when it comes from ~/.hermes/.env; gh-cli / apps.json are left alone.
          await disableCopilot()
          // The server clears model.default when the default model is Copilot; here we
          // additionally clean up the local per-session model/provider so the Chat page
          // does not keep showing removed Copilot models.
        ...
        // After deletion, if there is no default model, auto-pick one from the remaining
        // providers to avoid the awkward "no default model" state on the chat page.
        // Matches the implicit behavior of the hermes CLI `model` subcommand.
