...
import { fetchLocalTtsStatus, removeLocalTtsModel, startLocalTtsDownload } from '@/api/hermes/tts-settings'
...
async function handleDownloadLocal(conn: VoiceApiConnection) {
  setCardTestState(conn.id, 'loading', t('settings.voice.localDownloadStarting'))
  try {
    const res = await startLocalTtsDownload()
    if (!res.ok) {
      setCardTestState(conn.id, 'error', res.error || t('settings.voice.localDownloadFailed'))
      return
    }
    setCardTestState(conn.id, 'success', t('settings.voice.localDownloadComplete'))
    await voiceApi.refresh()
  } catch (err) {
    setCardTestState(conn.id, 'error', err instanceof Error ? err.message : String(err))
  } finally {
    if (localStatusPollHandle !== null) {
      window.clearInterval(localStatusPollHandle)
      localStatusPollHandle = null
    }
  }
}

async function handleRemoveLocal(conn: VoiceApiConnection) {
  try {
    await removeLocalTtsModel()
    await voiceApi.refresh()
    setCardTestState(conn.id, 'success', t('settings.voice.localRemoveComplete'))
  } catch (err) {
    setCardTestState(conn.id, 'error', err instanceof Error ? err.message : String(err))
  }
}

function handleSelectModel(conn: VoiceApiConnection, _modelId: string) {
  // ponytail: dropdown is informational for now — the engine-manager still
  // downloads the hardcoded default. Wire the selection through to the
  // download flow when multi-model support lands.
  console.debug('[VoiceSettings] model selected', conn.id, _modelId)
}

// Poll the local model status while a download is in flight so the progress
// bar updates without a full page reload. Covers both pip install (installing)
// and the actual model download (downloading).
let localStatusPollHandle: number | null = null
async function pollLocalStatusWhileDownloading() {
  if (localStatusPollHandle !== null) return
  const tick = async () => {
    try {
      const status = await fetchLocalTtsStatus()
      const isDone = status?.model?.status === 'ready' || status?.model?.status === 'error'
      if (isDone) {
        if (localStatusPollHandle !== null) {
          window.clearInterval(localStatusPollHandle)
          localStatusPollHandle = null
  ...
  await tick()
  localStatusPollHandle = window.setInterval(tick, 1500)
}
...
function openaiOptionsFor(connection: VoiceApiConnection): OpenaiTtsOptions {
  ...
  const provider = connection.provider === 'openai' || connection.provider === 'custom' || connection.provider === 'doubao'
    ? connection.provider
    : undefined
  return {
...
function localOptionsFor(connection: VoiceApiConnection): { provider: 'local'; voice?: string; speed?: number } {
  const options = ttsOptionsFor(connection)
...
async function handleTtsTest(connection: VoiceApiConnection) {
  ...
  try {
    ...
    } else if (connection.provider === 'local') {
      if (connection.modelState !== 'ready') {
        setCardTestState(connection.id, 'error', t('settings.voice.localModelNotReady'))
        return
      }
      await speech.openaiPlay(connection.id, text, localOptionsFor(connection) as unknown as OpenaiTtsOptions)
    } else if (connection.provider === 'openai' || connection.provider === 'custom' || connection.provider === 'doubao') {
...
<template>
  <div class="voice-settings">
    <section class="settings-section voice-provider-section" aria-labelledby="tts-providers-title">
      <header class="section-header">
        <div class="section-copy">
          <span class="section-eyebrow">{{ t('settings.voice.ttsSectionLabel') }}</span>
      ...
      <div class="provider-list">
        <VoiceApiCard
          ...
          @remove="handleRemove"
          @download-local="(c) => { handleDownloadLocal(c); pollLocalStatusWhileDownloading() }"
    ...
    <section class="settings-section voice-provider-section" aria-labelledby="stt-providers-title">
      <header class="section-header">
        <div class="section-copy">
          <span class="section-eyebrow">{{ t('settings.voice.sttSectionLabel') }}</span>
