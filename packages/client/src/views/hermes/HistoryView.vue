...
const contextMenuOptions = computed<DropdownOption[]>(() => {
  const options: DropdownOption[] = [
    {
      label: t('chat.importToUi'),
      key: 'import-ui',
      disabled: Boolean(contextSessionSummary.value?.ui_imported),
    },
...
async function handleImportToUi(sessionId: string) {
  const summary = findHistorySession(sessionId)
  try {
...
async function handleContextMenuSelect(key: string) {
  ...
  } else if (key === 'import-ui') {
    await handleImportToUi(contextSessionId.value)
