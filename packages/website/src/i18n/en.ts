export default {
  ...
  hero: {
    ...
    docsLibrary: 'Library',
    install: 'npm install -g @pythoughts/hermes-ui',
    previewAlt: 'Hermes UI dashboard preview',
  },
  features: {
    title: 'One Local Console for Hermes Agent',
    desc: 'Hermes UI brings chat, runtime management, automation, files, coding agents, and release-ready desktop distribution into one workspace.',
    streaming: {
      title: 'Agent Chat',
    ...
    quickInstall: {
      ...
      desc: 'Windows, macOS, and Linux desktop builds bundle the UI runtime and manage Hermes UI state separately from Hermes Agent data.',
    },
    i18n: {
      title: 'Coding Agents & MCP',
      desc: 'Launch local coding-agent sessions, use Codex and Claude Code proxy routes, and manage the bundled hermes-ui MCP server.',
    },
    theme: {
  ...
  screenshots: {
    ...
    items: [
      {
        src: '/image.gif',
        alt: 'Hermes UI product walkthrough',
        title: 'Hermes UI Workflow',
  ...
  install: {
    title: 'Quick Start',
    desc: 'Download the desktop app or run Hermes UI yourself.',
    desktop: {
      title: 'Desktop',
      download: 'Download',
      githubDownload: 'GitHub Download',
      allDownloads: 'View all release assets',
      prereq: 'Desktop builds bundle the Hermes UI runtime.',
      downloads: [
        {
    ...
    npm: {
      title: 'npm',
      cmd1: 'npm install -g @pythoughts/hermes-ui',
      cmd2: 'hermes-ui start',
    },
    docker: {
      title: 'Docker',
    ...
    source: {
      title: 'From Source',
      cmd1: 'git clone https://github.com/Pythoughts-labs/hermes-ui.git',
      cmd2: 'cd hermes-ui && npm install && npm run dev',
    },
  ...
  docs: {
    ...
    gettingStarted: {
      ...
      intro: 'Hermes UI is a self-hosted web dashboard for managing AI conversations, platform channels, scheduled jobs, and more. It wraps the Hermes Agent CLI and provides a beautiful web interface.',
      install: {
        title: 'Installation',
      ...
      firstRun: {
        title: 'First Run',
        content: 'On first start, Hermes UI will automatically generate an auth token, initialize local data, start the Hermes agent bridge, and open the dashboard in your browser.',
      },
      login: {
        title: 'Login',
        content: 'The auto-generated token is stored in ~/.hermes-ui/.token. Username/password login is available with bootstrap credentials admin / admin on first use, and the app prompts users to change default credentials after login.',
      },
    },
    configuration: {
      title: 'Configuration',
      intro: 'Hermes UI can be configured via environment variables.',
      envVars: {
        ...
        rows: [
          ...
          ['HERMES_UI_HOME', 'Hermes UI data home for auth token, credentials, logs, DB, and default uploads'],
          ['HERMES_UI_STATE_DIR', 'Compatibility alias for HERMES_UI_HOME'],
          ['HERMES_UI_DISABLE_MCP_AUTOINJECT', 'Disable startup injection of the managed hermes-ui MCP server into Hermes profile configs'],
          ['HERMES_UI_ALLOW_TRANSIENT_MCP_AUTOINJECT', 'Allow managed MCP injection when HERMES_UI_HOME is under a temporary directory, such as Version Preview runtimes'],
          ...
          ['HERMES_AGENT_BRIDGE_STOP_ON_SHUTDOWN', 'Stop the bridge broker during UI shutdown and restart by default; set 0/false/no/off to preserve the broker across restarts'],
          ...
          ['HERMES_UI_MANAGED_GATEWAY', 'Controls UI-managed Hermes gateway handling. Enabled by default; set 0/false/off to use hermes gateway start'],
          ['HERMES_UI_DISABLE_GATEWAY_AUTOSTART', 'Skip startup gateway checks/autostart for dashboard-only deployments where another service owns Hermes gateway lifecycle'],
          ['HERMES_UI_DISABLE_SKILL_INJECTION', 'Skip startup bundled skill injection when skills are managed outside Hermes UI. Enabled injection only updates UI-managed or identical bundled copies; local edits are skipped'],
          ['HERMES_UI_STOP_GATEWAYS_ON_SHUTDOWN', 'Controls whether Hermes UI shutdown also stops managed gateway processes'],
          ...
          ['HERMES_UI_PREVIEW_REPO', 'GitHub repository used by Version Preview'],
          ['HERMES_UI_PREVIEW_AGENT_BRIDGE_TRANSPORT', 'Version Preview broker endpoint transport. Set tcp to use loopback TCP for Preview on macOS/Linux; when unset, Preview follows HERMES_AGENT_BRIDGE_WORKER_TRANSPORT=tcp'],
          ['HERMES_UI_PREVIEW_AGENT_BRIDGE_ENDPOINT', 'Directly overrides the Version Preview broker endpoint for deployments that need a fully custom Preview bridge address'],
          ['HERMES_UI_BACKEND_PORT', 'Backend port used by the Vite dev proxy'],
          ['HERMES_UI_FRONTEND_PORT', 'Frontend Vite dev server port'],
        ],
      },
      gateway: {
        title: 'Agent Bridge Runtime',
        content: 'Chat runs are handled through the Hermes agent bridge, which runs alongside the Hermes UI server and talks directly to the Hermes Agent runtime. HERMES_AGENT_BRIDGE_ENDPOINT controls the Node-to-broker address, while HERMES_AGENT_BRIDGE_WORKER_TRANSPORT controls the broker-to-profile-worker transport. UI shutdown and CLI/app restarts stop the bridge broker by default; set HERMES_AGENT_BRIDGE_STOP_ON_SHUTDOWN=0 only when you intentionally want to preserve bridge sessions across restart. Switching the frontend Hermes Profile changes later request context only; it does not restart the bridge or clear other running tasks.',
      },
      profiles: {
        title: 'Profiles',
    ...
    features: {
      title: 'Features',
      intro: 'Explore the core features of Hermes UI.',
      chat: {
        title: 'AI Chat',
        content: 'Real-time chat streaming over Socket.IO /chat-run. Supports multi-session management, Markdown rendering with syntax highlighting, tool call inspection, profile-scoped upload, path-based download, and Ctrl+K search across the Hermes UI local session database.',
      },
      kanban: {
        title: 'Kanban Board',
    ...
    hermesUiManual: {
      title: 'Hermes UI Client Manual',
      intro: 'The complete Hermes UI client operations manual is published on the official documentation site. It covers day-to-day usage, configuration, operations, visible navigation, dialogs, status pages, key actions, common workflows, and risk notes.',
      open: {
        title: 'Open the full manual',
        content: 'The full manual is hosted on the official documentation site. Use the link below for the latest version with annotated screenshots, workflow sections, tables, and operational notes.',
        links: [
          {
            label: 'Open the Hermes UI manual',
      ...
      scope: {
        title: 'Scope',
        rows: [
          ['Version', 'Hermes UI client operations manual'],
          ['Language', 'English'],
      ...
      maintenance: {
        ...
        content: 'When the website, reference docs, Hermes agent docs, and the current client UI differ, the manual treats the currently visible client interface as the operational source of truth. For future releases, update the documentation site scope.',
      },
    },
    esp32Intro: {
      title: 'Hermes ESP32-C3 Device Intro',
      intro: 'The Hermes ESP32-C3 is a desktop AI conversation box that handles the small display, microphone, speaker, physical buttons, and local network connection. Hermes UI handles model access, speech recognition, speech synthesis, and session execution.',
      open: {
        title: 'Open the full intro page',
        content: 'The full device introduction is published on the official documentation site, including product visuals, setup flow, specification notes, pre-purchase checklist, and FAQ.',
        links: [
          {
            label: 'Open the Hermes ESP32-C3 intro',
            href: 'https://github.com/Pythoughts-labs/hermes-ui',
            description: 'Includes product visuals, hardware highlights, Wi-Fi provisioning notes, Hermes UI connection guidance, specifications, and FAQ.',
          },
        ],
      },
      overview: {
        title: 'Device positioning',
        content: 'This is a desktop hardware entry point for Hermes UI, not an offline standalone large-model device. The ESP32-C3 provides local interaction and status feedback, while AI capability comes from Hermes UI running on a computer or server plus the model, STT, and TTS services configured by the user.',
      },
      requirements: {
        title: 'Requirements',
        rows: [
          ['Network', 'The ESP32 device and Hermes UI should communicate on the same local network.'],
          ['Service address', 'Enter the computer or server LAN IP plus port on the device page; do not use 127.0.0.1.'],
          ['Voice capability', 'Hermes UI needs working speech recognition, speech synthesis, and model services.'],
          ['Best fit', 'Built for Hermes UI users, ESP32/Arduino/PlatformIO makers, open hardware customization, and desktop AI interaction demos.'],
        ],
      },
      maintenance: {
        title: 'Maintenance note',
        content: 'If firmware behavior, provisioning pages, ports, device visuals, or the Hermes UI device connection flow changes, update the documentation site scope.',
      },
    },
    platforms: {
    ...
    api: {
      title: 'API Reference',
      intro: 'Hermes UI provides a local BFF API for the dashboard and Socket.IO endpoints for streaming chat.',
      local: {
      ...
      auth: {
        ...
        content: 'API endpoints require authenticated access. The token is auto-generated on first run and stored in ~/.hermes-ui/.token. Username/password login uses account records; super administrators manage users and profile bindings, while regular administrators manage their own account details.',
      },
    },
  },
}
