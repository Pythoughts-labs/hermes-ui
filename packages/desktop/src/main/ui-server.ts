...
export async function startUiServer(port = DEFAULT_PORT): Promise<string> {
  ...
  const entry = uiServerEntry()
  if (!existsSync(entry)) {
    throw new Error(`UI server entry not found at ${entry}. Run: npm run build:ui`)
  }
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    // Desktop opens the UI as soon as the UI HTTP server is ready, while
    ...
    HERMES_UI_PREVIEW_AGENT_BRIDGE_TRANSPORT: 'tcp',
    // Suppress the npm-registry update prompt (upstream #1105). hermes-ui
    // is bundled here; users can't `npm i -g` to upgrade, they have to wait
    // for the wrapper app to ship a new release.
    HERMES_UI_DISABLE_UPDATE_CHECK: 'true',
    ...
    // Keep the bundled Hermes Agent, bridge, gateway, and UI path helpers
  ...
  serverProc.on('exit', (code, signal) => {
    console.error(`[ui] server exited code=${code} signal=${signal}`)
    serverProc = null
  ...
  if (fullStartupTimeoutMs > 0) {
    await Promise.race([
      ...
      console.warn(`[ui] agent bridge was not ready during startup: ${err instanceof Error ? err.message : String(err)}`)
    })
...
async function waitForReady(port: number, timeoutMs: number): Promise<void> {
  ...
  throw new Error(`UI shell did not become ready within ${timeoutMs}ms`)
}
...
export async function stopUiServer(): Promise<void> {
  ...
  try {
    ...
    console.warn(`[ui] graceful shutdown request failed: ${err instanceof Error ? err.message : String(err)}`)
    killProcessTree(proc)
  }
