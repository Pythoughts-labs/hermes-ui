...
const SHIM_MARKER = 'HERMES_UI_CLI_SHIM'
const MCP_SHIM_MARKER = 'HERMES_UI_MCP_SHIM'
const PATH_MARKER_START = '# >>> Hermes UI CLI shim >>>'
const PATH_MARKER_END = '# <<< Hermes UI CLI shim <<<'
const WINDOWS_USER_PATH_ENV_B64 = 'HERMES_UI_WINDOWS_USER_PATH_B64'
...
export function shimPathForPlatform(binDir: string, platform: NodeJS.Platform = process.platform): string {
  return join(binDir, platform === 'win32' ? 'hermes-ui.cmd' : 'hermes-ui')
}

export function mcpShimPathForPlatform(binDir: string, platform: NodeJS.Platform = process.platform): string {
  return join(binDir, platform === 'win32' ? 'hermes-ui-mcp.cmd' : 'hermes-ui-mcp')
}
...
export function createShimContent(
  ...
  uiScriptPath = resolve(process.cwd(), 'bin', 'hermes-ui.mjs'),
): string {
  if (platform === 'win32') {
    ...
    const webForwarder = `const cp=require('node:child_process');const args=process.argv.slice(1);if(args[0]&&args[0].toLowerCase()==='web')args.shift();const r=cp.spawnSync(process.env.NODE,[process.env.UI_SCRIPT,...args],{stdio:'inherit'});if(r.error){console.error(r.error.message);process.exit(127)}process.exit(r.status===null?(r.signal?1:0):r.status)`
    return [
      '@echo off',
      `rem ${SHIM_MARKER}`,
      `set "APP=${executablePath}"`,
      `set "NODE=${nodePath}"`,
      `set "UI_SCRIPT=${uiScriptPath}"`,
      'if "%~1"=="" goto openApp',
      ...
      'echo Unknown Hermes UI command: %~1 1>&2',
      'echo Run hermes-ui --help for usage. 1>&2',
      'exit /b 2',
      ':resolveRuntime',
      'set "UI_HOME=%HERMES_UI_HOME%"',
      'if "%UI_HOME%"=="" set "UI_HOME=%HERMES_UI_STATE_DIR%"',
      'if "%UI_HOME%"=="" set "UI_HOME=%USERPROFILE%\\.hermes-ui"',
      'set "RUNTIME=%HERMES_DESKTOP_RUNTIME_DIR%"',
      'if "%RUNTIME%"=="" if exist "%UI_HOME%\\desktop-runtime\\active-version.json" (',
      `  for /f "usebackq delims=" %%I in (\`powershell.exe -NoProfile -ExecutionPolicy Bypass -Command "$p = Join-Path $env:UI_HOME 'desktop-runtime\\active-version.json'; try { $j = Get-Content -LiteralPath $p -Raw | ConvertFrom-Json; if ($j.platform -eq '${runtimePlatform}' -and $j.runtimeDirectory -and (Test-Path -LiteralPath $j.runtimeDirectory)) { [Console]::Out.Write($j.runtimeDirectory) } } catch {}" 2^>nul\`) do set "RUNTIME=%%I"`,
      ')',
      `if "%RUNTIME%"=="" set "RUNTIME=%UI_HOME%\\desktop-runtime\\hermes\\${runtimeVersion}\\${runtimePlatform}"`,
      'set "PYTHON=%RUNTIME%\\python\\python.exe"',
      'exit /b 0',
      ':runCli',
      'call :resolveRuntime',
      'if not exist "%PYTHON%" (',
      '  echo Hermes UI Python runtime not found at "%PYTHON%" 1>&2',
      '  echo Open Hermes UI once to finish runtime setup, then retry hermes-ui cli. 1>&2',
      '  exit /b 127',
      ')',
      'if not exist "%NODE%" (',
      '  echo Hermes UI Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes UI once to finish runtime setup, then retry hermes-ui cli. 1>&2',
      '  exit /b 127',
      ')',
      `"%NODE%" -e "${cliForwarder}" %*`,
      ...
      'if not exist "%NODE%" (',
      '  echo Hermes UI Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes UI once to finish runtime setup, then retry hermes-ui web. 1>&2',
      '  exit /b 127',
      ')',
      'if not exist "%UI_SCRIPT%" (',
      '  echo Hermes UI script not found at "%UI_SCRIPT%" 1>&2',
      '  exit /b 127',
      ')',
      `"%NODE%" -e "${webForwarder}" %*`,
      'exit /b %ERRORLEVEL%',
      ...
      'exit /b 0',
      ':help',
      'echo Usage: hermes-ui [command] [options]',
      'echo.',
      'echo Commands:',
      'echo   ^(no command^)       Open Hermes UI desktop app',
      'echo   cli [args...]       Run bundled Hermes Agent CLI',
      'echo   web [args...]       Run bundled hermes-ui command',
  ...
  return [
    ...
    `UI_SCRIPT=${shellQuote(uiScriptPath)}`,
    'show_help() {',
    '  cat <<\'EOF\'',
    'Usage: hermes-ui [command] [options]',
    '',
    'Commands:',
    '  (no command)       Open Hermes UI desktop app',
    '  cli [args...]      Run bundled Hermes Agent CLI',
    '  web [args...]      Run bundled hermes-ui command',
    '  help, -h, --help   Show this help message',
    'EOF',
    '}',
    'if [ ! -x "$APP" ]; then',
    '  echo "Hermes UI executable not found at $APP" >&2',
    '  exit 127',
    'fi',
    ...
    '    shift',
    '    if [ ! -x "$NODE" ]; then',
    '      echo "Hermes UI Node runtime not found at $NODE" >&2',
    '      echo "Open Hermes UI once to finish runtime setup, then retry hermes-ui web." >&2',
    '      exit 127',
    '    fi',
    '    if [ ! -f "$UI_SCRIPT" ]; then',
    '      echo "Hermes UI script not found at $UI_SCRIPT" >&2',
    '      exit 127',
    '    fi',
    '    exec "$NODE" "$UI_SCRIPT" "$@"',
    '    ;;',
    ...
    '    ;;',
    '  *)',
    '    echo "Unknown Hermes UI command: $1" >&2',
    '    echo "Run hermes-ui --help for usage." >&2',
    '    exit 2',
    '    ;;',
    'esac',
    '',
...
export function createMcpShimContent(
  ...
  if (platform === 'win32') {
    return [
      ...
      'if not exist "%NODE%" (',
      '  echo Hermes UI Node runtime not found at "%NODE%" 1>&2',
      '  echo Open Hermes UI once to finish runtime setup, then retry hermes-ui-mcp. 1>&2',
      '  exit /b 127',
      ')',
      'if not exist "%SCRIPT%" (',
      '  echo Hermes UI MCP script not found at "%SCRIPT%" 1>&2',
      '  exit /b 127',
      ')',
      'if "%HERMES_UI_URL%"=="" (',
      '  if "%HERMES_DESKTOP_PORT%"=="" (',
      `    set "HERMES_UI_URL=${uiUrl}"`,
      '  ) else (',
      '    set "HERMES_UI_URL=http://127.0.0.1:%HERMES_DESKTOP_PORT%"',
      '  )',
      ')',
      'if "%HERMES_MCP_SERVER_NAME%"=="" set "HERMES_MCP_SERVER_NAME=hermes-ui-mcp"',
      '"%NODE%" "%SCRIPT%" %*',
      'exit /b %ERRORLEVEL%',
      '',
    ].join('\r\n')
  ...
  return [
    ...
    'if [ ! -x "$NODE" ]; then',
    '  echo "Hermes UI Node runtime not found at $NODE" >&2',
    '  echo "Open Hermes UI once to finish runtime setup, then retry hermes-ui-mcp." >&2',
    '  exit 127',
    'fi',
    'if [ ! -f "$SCRIPT" ]; then',
    '  echo "Hermes UI MCP script not found at $SCRIPT" >&2',
    '  exit 127',
    'fi',
    'if [ -z "${HERMES_UI_URL:-}" ]; then',
    '  if [ -n "${HERMES_DESKTOP_PORT:-}" ]; then',
    '    HERMES_UI_URL="http://127.0.0.1:${HERMES_DESKTOP_PORT}"',
    '  else',
    `    HERMES_UI_URL=${shellQuote(uiUrl)}`,
    '  fi',
    'fi',
    'export HERMES_UI_URL',
    'if [ -z "${HERMES_MCP_SERVER_NAME:-}" ]; then',
    '  HERMES_MCP_SERVER_NAME=hermes-ui-mcp',
    'fi',
...
function shellProfilePaths(homeDir: string, platform: NodeJS.Platform, env: NodeJS.ProcessEnv): string[] {
  ...
  if (name === 'fish') return [join(homeDir, '.config', 'fish', 'conf.d', 'hermes-ui.fish')]
...
export async function installHermesUiCliShim(options: CliShimInstallOptions = {}): Promise<CliShimInstallResult> {
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-ui shim is not managed by Hermes UI' : undefined,
  }
}

export async function installHermesUiMcpShim(options: McpShimInstallOptions = {}): Promise<CliShimInstallResult> {
  const platform = options.platform || process.platform
  const env = options.env || process.env
  const homeDir = options.homeDir || homedir()
  const binDir = resolve(homeDir, 'bin')
  ...
  const scriptPath = options.scriptPath || resolve(process.cwd(), 'bin', 'hermes-ui-mcp.mjs')
  const uiUrl = options.uiUrl || 'http://127.0.0.1:8748'

  mkdirSync(binDir, { recursive: true })
  const status = writeShim(shimPath, createMcpShimContent(nodePath, scriptPath, uiUrl, platform), platform, MCP_SHIM_MARKER)
  const pathUpdated = await ensureUserBinOnPath(homeDir, binDir, platform, env).catch((err) => {
    console.warn(`[cli-shim] failed to update PATH: ${err instanceof Error ? err.message : String(err)}`)
    return false
  })
  ...
  return {
    ...
    status,
    pathUpdated,
    reason: status === 'skipped' ? 'existing hermes-ui-mcp shim is not managed by Hermes UI' : undefined,
  }
}
