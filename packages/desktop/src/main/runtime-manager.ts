...
const DEFAULT_RUNTIME_GITHUB_REPO = 'Pythoughts-labs/hermes-ui'
...
async function resolveRuntimeDescriptor(): Promise<RuntimeDescriptor> {
  ...
  if (!manifestOverride) {
    throw new Error('Hermes runtime manifest URL is not configured')
  }
  ...
  for (const candidate of candidates) {
    try {
      console.log(`[runtime] resolving Hermes runtime from custom: ${candidate.url}`)
...
function uiVersion(): string {
  const packageJson = join(uiDir(), 'package.json')
  try {
...
export function writeActiveRuntimeVersion(runtimeRoot = desktopRuntimeDir()): void {
  ...
  const activeVersionPath = join(uiHome(), 'desktop-runtime', ACTIVE_RUNTIME_VERSION_NAME)
