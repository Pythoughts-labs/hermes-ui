...
import { startUiServer, stopUiServer, getToken } from './ui-server'
import { bundledNode, desktopIcon, desktopRuntimeVersion, desktopTrayTemplateIcon, desktopWindowsTrayIcon, hermesBinExists, hermesBin, uiDir } from './paths'
import { checkForDesktopUpdates, initAutoUpdater } from './updater'
import { t } from './desktop-i18n'
import { installHermesUiCliShim, installHermesUiMcpShim } from './cli-shim'
...
const APP_USER_MODEL_ID = 'com.pythoughtslabs.hermesui'
...
function createWindow() {
  ...
  // If the UI server is already up (re-opening window after close on
...
function splashHtml(label = t('desktop.startingLocalServices')): string {
  ...
  const html = `<!doctype html><html><head><meta charset="utf-8"><title>Hermes UI</title>
<style>
...
function resolveRuntimeSourceLogo(): string {
  const candidates = [
    join(uiDir(), 'dist', 'client', 'logo.png'),
    join(uiDir(), 'packages', 'client', 'public', 'logo.png'),
    join(uiDir(), 'logo.png'),
    desktopIcon(),
  ]
...
<div class="brand">${logoUrl ? `<img class="mark" src="${logoUrl}" alt="Hermes UI">` : ''}<h1>Hermes UI</h1></div>
...
async function bootstrap() {
  ...
  try {
    ...
    if (needsRuntimeWork) {
      if (!runtimeUrlOverride && !manifestOverride) {
...
function resolveNotificationIcon(icon: unknown): string {
  ...
  const candidates = [
    join(uiDir(), 'dist', 'client', normalized),
    join(uiDir(), 'dist', normalized),
    join(uiDir(), 'packages', 'client', 'public', normalized),
    join(uiDir(), normalized),
  ]
  return candidates.find(candidate => existsSync(candidate)) || desktopIcon()
}
...
function runDesktopApp() {
  ...
  app.whenReady().then(() => {
    ...
    if (app.isPackaged) {
      installHermesUiCliShim({
        ...
        uiScriptPath: join(uiDir(), 'bin', 'hermes-ui.mjs'),
      }).then(result => {
        if (result.status === 'skipped') {
          console.warn(`[cli-shim] ${result.reason}: ${result.shimPath}`)
        }
      }).catch(err => {
        console.warn(`[cli-shim] failed to install hermes-ui command: ${err instanceof Error ? err.message : String(err)}`)
      })
      installHermesUiMcpShim({
        nodePath: bundledNode(),
        scriptPath: join(uiDir(), 'bin', 'hermes-ui-mcp.mjs'),
        ...
        console.warn(`[cli-shim] failed to install hermes-ui-mcp command: ${err instanceof Error ? err.message : String(err)}`)
      })
    }
    createTray()
    createWindow()
  ...
  app.on('before-quit', async (e) => {
    ...
    await stopUiServer().catch(() => undefined)
    app.exit(0)
  })
}
