...
const GITHUB_LATEST_FEED_URL = 'https://github.com/Pythoughts-labs/hermes-ui/releases/latest/download'
...
$target = [System.IO.Path]::GetFullPath($env:HERMES_UI_UPDATE_EXE)
$current = [int]$env:HERMES_UI_UPDATE_PID
function Get-HermesUiProcess {
...
Get-HermesUiProcess | ForEach-Object {
  try {
...
`.trim()
  try {
    await execFileAsync('powershell.exe', ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-Command', script], {
      env: {
        ...process.env,
        HERMES_UI_UPDATE_EXE: normalizedExecPath,
        HERMES_UI_UPDATE_PID: String(currentPid),
      },
      timeout: 30_000,
      windowsHide: true,
    })
