...
function defaultUiDir(): string {
  if (isPackaged()) return resolve(process.resourcesPath, 'ui')
  return process.env.HERMES_UI_DIR?.trim() || resolve(app?.getAppPath?.() || resolve(process.cwd(), 'packages', 'desktop'), '..', '..')
}
...
function installedRuntimeDirectories(): Array<{ directory: string; version: string }> {
  const root = join(uiHome(), 'desktop-runtime', 'hermes')
...
function activeRuntimeVersionFile(): string {
  return join(uiHome(), 'desktop-runtime', ACTIVE_RUNTIME_VERSION_NAME)
}
...
function cleanupLegacyUiVersions(): void {
  if (legacyUiVersionsCleaned) return
  legacyUiVersionsCleaned = true

  const root = join(uiHome(), 'ui')
  if (!existsSync(root)) return
  ...
  try {
    for (const entry of readdirSync(root, { withFileTypes: true })) {
      ...
      const comparison = compareHermesAgentVersions(version, MIN_COMPATIBLE_UI_VERSION)
      if (comparison === null || comparison >= 0) continue
      const target = join(root, entry.name)
      rmSync(target, { recursive: true, force: true })
      console.log(`[desktop] removed incompatible UI cache ${version}: ${target}`)
    }
  } catch (err) {
    console.warn('[desktop] failed to clean incompatible UI caches:', err instanceof Error ? err.message : String(err))
  }
}

// Bundled ui directory.
// dev:  <repo root> (or HERMES_UI_DIR)
// prod: <resources>/ui
// active-version.json can pin the UI path used to start the local server.
export function uiDir(): string {
  const override = process.env.HERMES_UI_DIR?.trim()
  if (override) return resolve(override)

  cleanupLegacyUiVersions()

  const active = readActiveRuntimeVersion()
  if (active?.platform === runtimePlatformKey()
    && typeof active.uiDirectory === 'string'
    && active.uiDirectory.trim()
    && existsSync(active.uiDirectory)) {
    return resolve(active.uiDirectory)
  }

  return defaultUiDir()
}

export function uiServerEntry(): string {
  return join(uiDir(), 'dist', 'server', 'index.js')
}
...
export function targetDesktopRuntimeDir(): string {
  ...
  if (override) return resolve(override)
  return join(uiHome(), 'desktop-runtime', 'hermes', desktopRuntimeVersion(), runtimePlatformKey())
}
...
// prod: downloaded runtime cache under UI home.
...
export function uiHome(): string {
  return process.env.HERMES_UI_HOME?.trim() || resolve(homedir(), '.hermes-ui')
}
