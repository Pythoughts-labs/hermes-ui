...
const EXTRA_PYTHON_PACKAGES = splitPackageList(
  process.env.HERMES_EXTRA_PYTHON_PACKAGES || [
    ...
    'aiosqlite==0.22.1',
    'asyncpg==0.31.0',
    // Local TTS (Kokoro-82M). STANDARD: install kokoro AND soundfile together.
    // soundfile is NOT a kokoro dependency (kokoro's Requires-Dist omits it)
    // but hermes_tts_engine.py needs it to WAV-encode — omitting it crashes
    // Test with "No module named 'soundfile'". Keep this in sync with
    // engine-manager.ts KOKORO_PACKAGES and scripts/setup-local-tts.sh.
    // espeak-ng is a system-level dep installed by the packaging step.
    'kokoro>=0.9.4',
    'soundfile',
...
run(pyBin, [
  ...
  [
    ...
    // defensive: verify BOTH local-TTS deps landed — a packaging run that
    // exits 0 but leaves soundfile uninstalled ships a broken Test button.
...
// hermes-ui's agent-bridge searches for `run_agent.py` at <python_root>/run_agent.py
...
// Bundle the local TTS engine script into the runtime so the desktop UI
// can spawn `python3 hermes_tts_engine.py` without depending on a relative
// path into the dev repo.
const localTtsEngineSrc = resolve(__dirname, 'python-assets', 'hermes_tts_engine.py')
if (existsSync(localTtsEngineSrc)) {
  const localTtsEngineDst = TARGET_OS === 'win32'
    ? resolve(PY_DIR, 'hermes_tts_engine.py')
    : resolve(PY_DIR, 'bin', 'hermes_tts_engine.py')
  copyFileSync(localTtsEngineSrc, localTtsEngineDst)
  if (TARGET_OS !== 'win32') chmodSync(localTtsEngineDst, 0o755)
  console.log(`✓ local TTS engine bundled at ${localTtsEngineDst}`)
}
