...
This document defines project-level development rules for Hermes UI. It is tool-agnostic and applies to all contributors and coding agents.
...
- UI state lives under `HERMES_UI_HOME` or `HERMES_UI_STATE_DIR`, defaulting to `~/.hermes-ui`.
...
- Use `config.appHome` for UI state paths.
- Keep Hermes home paths separate from UI home paths.
...
- Keep PR titles concrete and scoped, for example `[codex] make UI state directory configurable`.
